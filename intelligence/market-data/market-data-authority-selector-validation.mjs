...
const pilot = ['bitcoin', 'ethereum', 'aerodrome-finance', 'pendle', 'curve-dao-token', 'frax-share'];
...
if (JSON.stringify(policy.pilot?.assetIds) !== JSON.stringify(pilot)) throw new Error('Pilot cohort must be exactly BTC + ETH + AERO + PENDLE + CRV + FXS');
if (Number(policy.pilot?.maxPromotedAssetCount) !== 6) throw new Error('Pilot cap must be 6');
...
if (healthy.coverage.onchainSelectedCount !== 6) throw new Error('Healthy pilot must select exactly 6 direct-oracle assets onchain');
if (healthy.coverage.coingeckoSelectedCount !== healthy.coverage.assetCount - 6) throw new Error('Non-pilot assets must remain CoinGecko');
