...
const pilot = ['bitcoin', 'ethereum', 'aerodrome-finance', 'pendle', 'curve-dao-token', 'frax-share', 'venice-token', 'internet-computer'];
...
if (JSON.stringify(policy.pilot?.assetIds) !== JSON.stringify(pilot)) throw new Error('Pilot cohort must be exactly BTC + ETH + AERO + PENDLE + CRV + FXS + VVV + ICP');
if (Number(policy.pilot?.maxPromotedAssetCount) !== 8) throw new Error('Pilot cap must be 8');
...
if (healthy.coverage.onchainSelectedCount !== 8) throw new Error('Healthy pilot must select exactly 8 direct-oracle assets onchain');
if (healthy.coverage.coingeckoSelectedCount !== healthy.coverage.assetCount - 8) throw new Error('Non-pilot assets must remain CoinGecko');
