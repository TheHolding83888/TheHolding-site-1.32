...
const pilot = ['bitcoin', 'ethereum', 'aerodrome-finance', 'pendle', 'curve-dao-token', 'frax-share', 'venice-token', 'internet-computer', 'decentraland', 'the-sandbox'];
...
if (JSON.stringify(policy.pilot?.assetIds) !== JSON.stringify(pilot)) throw new Error('Pilot cohort must be exactly BTC + ETH + AERO + PENDLE + CRV + FXS + VVV + ICP + MANA + SAND');
if (Number(policy.pilot?.maxPromotedAssetCount) !== 10) throw new Error('Pilot cap must be 10');
...
if (healthy.coverage.onchainSelectedCount !== 10) throw new Error('Healthy pilot must select exactly 10 direct-oracle assets onchain');
if (healthy.coverage.coingeckoSelectedCount !== healthy.coverage.assetCount - 10) throw new Error('Non-pilot assets must remain CoinGecko');
