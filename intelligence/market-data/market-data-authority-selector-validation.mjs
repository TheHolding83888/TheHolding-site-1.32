...
// Every canonical route must fail back only to the cached daily CoinGecko lane when its
// own onchain observation is unhealthy. This proves a route incident cannot zero the site.
for (const id of promoted) {
...
// Dependency-aware routes must reject semantic dependency drift, not just RPC failure.
for (const id of promoted.filter(id => policy.assetOverrides?.[id]?.requiredDependencyStatus)) {
  ...
  badDependency.observations[id] = { ...badDependency.observations[id], dependencyStatus: 'rpc-unavailable' };
  const fallback = selectMarketDataAuthority({ policy, marketData: source, shadow: badDependency, assetId: id, nowMs });
  if (fallback.selectedLane !== 'coingecko-lane' || fallback.fallbackUsed !== true) throw new Error(`${id}: dependency drift did not fail back`);
}
