...
if (!writer?.writeCapable || writer.hasConcurrency) throw new Error('writer classification canary failed');
if (!report.findings.some(f => f.id === 'write-capable-without-concurrency' && f.workflow === 'writer')) throw new Error('missing concurrency finding canary failed');
...
if (!reader || reader.writeCapable) throw new Error('read-only negative canary failed');

console.log('Workflow Control Plane Canary PASS (writer / concurrency / workflow_run / dispatch / duplicate-writer / cycle / read-only)');
