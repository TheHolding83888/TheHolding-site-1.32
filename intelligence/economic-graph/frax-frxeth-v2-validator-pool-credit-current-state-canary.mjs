...
const TIMEOUT_BLOCK=21_424_300n;
...
function makeFetch({allFail=false,block=BLOCK,historyTimeoutAbove=null,historyRangeLimit=30n}={}){
  return async (url,options)=>{
    ...
    const rows=payload.map(req=>{
      ...
      if(req.method==='eth_getLogs'){
        ...
        const filter=req.params[0],from=BigInt(filter.fromBlock),to=BigInt(filter.toBlock),span=to-from+1n;
        if(historyTimeoutAbove!==null&&span>BigInt(historyTimeoutAbove))throw new Error('The operation was aborted due to timeout');
        if(historyRangeLimit!==null&&span>BigInt(historyRangeLimit))return {jsonrpc:'2.0',id:req.id,error:{code:-32005,message:'block range too wide for synthetic archive provider'}};
...
const timeoutRecovered=await collectFraxFrxEthV2ValidatorPoolCreditCurrentState({registry,rpcRegistry:rpcRegistry(),historyRpcRegistry:historyRpcRegistry(),fetchImpl:makeFetch({block:TIMEOUT_BLOCK,historyTimeoutAbove:20_000n,historyRangeLimit:20_000n}),checkpoint:{blockTag:quantity(TIMEOUT_BLOCK)}});
assert.equal(timeoutRecovered.status,'ok');assert.equal(timeoutRecovered.measurementClass,'MEASURED');assert.equal(timeoutRecovered.rpc.endpointId,'second-ok');assert.equal(timeoutRecovered.rpc.logDiscovery.endpointId,'history-ok');assert.equal(timeoutRecovered.rpc.logDiscovery.timeoutObservedCount,1);assert.equal(timeoutRecovered.rpc.logDiscovery.timeoutChunkFallbackCount,1);assert.equal(timeoutRecovered.rpc.logDiscovery.timeoutChunkBlocks,20_000);assert.equal(timeoutRecovered.rpc.logDiscovery.requestCount,3);assert.equal(timeoutRecovered.rpc.logDiscovery.adaptiveSplitCount,1);assert.ok(timeoutRecovered.rpc.logDiscovery.smallestSuccessfulSpanBlocks<=20_000);assert.equal(timeoutRecovered.coverage.completeVPoolDeployedHistory,true);assert.equal(timeoutRecovered.summary.deployedPoolCount,2);assert.equal(timeoutRecovered.epistemic.currentStateAuthorityEndpoint,'second-ok');assert.equal(timeoutRecovered.epistemic.historyTransportAuthority,'discovery-only');assert.equal(timeoutRecovered.epistemic.executionAuthority,'none');
