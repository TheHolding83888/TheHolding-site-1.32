...
const E18=10n**18n;
const sigHashes=Array.from({length:8},(_,i)=>`0x${String(i+1).repeat(64)}`);
...
function config(){return {
  version:'0.3-frax-fxb-series-registry-current-source-mixed-interface',
  officialSources:{docs:'synthetic',fxbV2LegacyInterfaceEvidence:'FRAX() supports legacy v1.1.0'},
  semantics:{originOnlyBackingMeasurement:true,currentSeriesOnly:true,legacySeriesExcludedFromCurrentCoverage:true,redemptionAssetIdentityFunctions:['token()','FRAX()'],mixedLegacyAndCurrentContractInterfacesAllowed:true,executionAuthority:'none'},
  networks:{ethereum:{chainId:1,lFrax:ETH_LFRAX},fraxtal:{chainId:252,lFrax:FXTL_LFRAX}},
  series:[
    {id:'E',label:'ETH legacy-interface FXB',originNetwork:'ethereum',originAddress:ETH_SERIES,bridgedMirrors:[],documentedMaturityDate:'2020-01-01'},
    {id:'F',label:'FXTL current-interface FXB',originNetwork:'fraxtal',originAddress:FXTL_SERIES,bridgedMirrors:[],documentedMaturityDate:'2030-01-01'}
  ],
...
function makeFetch({failFraxtal=false}={}){return async (url,options)=>{
  ...
  const out=payload.map(req=>{
    ...
    if(to===series&&data===selectors[0]){
      if(isEth)return {jsonrpc:'2.0',id:req.id,error:{code:3,message:'execution reverted: synthetic legacy has no token()'}};
      return {jsonrpc:'2.0',id:req.id,result:addressResult(lFrax)};
    }
    if(to===series&&data===selectors[1])return {jsonrpc:'2.0',id:req.id,result:addressResult(lFrax)};
    if(to===series&&data===selectors[2])return {jsonrpc:'2.0',id:req.id,result:uint(maturity)};
    if(to===series&&data===selectors[3])return {jsonrpc:'2.0',id:req.id,result:uint(900n*E18)};
    if(to===series&&data===selectors[4])return {jsonrpc:'2.0',id:req.id,result:uint(1000n*E18)};
    if(to===series&&data===selectors[5])return {jsonrpc:'2.0',id:req.id,result:uint(100n*E18)};
    if(to===series&&data===selectors[6])return {jsonrpc:'2.0',id:req.id,result:uint(isEth?1n:0n)};
    if(to===lFrax&&data.startsWith(selectors[7]))return {jsonrpc:'2.0',id:req.id,result:uint(900n*E18)};
...
assert.equal(currentRegistry.version,'0.3-frax-fxb-series-registry-current-source-mixed-interface');
...
assert.match(currentRegistry.officialSources.fxbV2LegacyInterfaceEvidence,/legacy v1\.1\.0/);
assert.deepEqual(currentRegistry.semantics.redemptionAssetIdentityFunctions,['token()','FRAX()']);
...
assert.deepEqual(measurement.coverage.redemptionIdentityInterfaceCounts,{'FRAX()':1,'token()':1});
assert.deepEqual(measurement.coverage.seriesWithBackingDeficit,[]);
assert.equal(measurement.series.find(x=>x.id==='E').proof.redemptionIdentityInterfaceProven,'FRAX()');
assert.equal(measurement.series.find(x=>x.id==='F').proof.redemptionIdentityInterfaceProven,'token()');
...
assert.equal(measurement.epistemic.redemptionAssetIdentity,'MEASURED-source-bounded-mixed-interface-token-or-legacy-FRAX');
...
assert.match(obs.surfaces.fxb.mechanicalRelations[0].from,/token\(\).*legacy FRAX\(\)/);
...
console.log('FRAX FXB MIXED-INTERFACE CURRENT SERIES CANARY PASS',{
  measuredSeries:measurement.coverage.measuredOriginSeriesCount,
  identityInterfaces:measurement.coverage.redemptionIdentityInterfaceCounts,
...
if(process.env.GITHUB_ACTIONS==='true'){
  ...
  if(live.series.some(x=>x.proof?.lFraxIdentityProven!==true||!['token()','FRAX()'].includes(x.proof?.redemptionIdentityInterfaceProven)||x.proof?.mintedMinusRedeemedEqualsSupply!==true||x.proof?.redeemabilityMatchesBlockTimestamp!==true))throw new Error('FRAX FXB live current-series proof invariant failed');
  ...
  console.log('FRAX FXB LIVE CURRENT-SERIES EXACT-BLOCK PROOF PASS',{
    ...
    identityInterfaces:live.coverage.redemptionIdentityInterfaceCounts,
    series:live.series.map(x=>({id:x.id,network:x.originNetwork,address:x.originAddress,identity:x.observed.redemptionIdentityFunction,maturity:x.observed.maturityIso,isRedeemable:x.observed.isRedeemable,supply:x.observed.totalSupplyLfraxFace,backing:x.observed.backingLfrax,backingDeficitRaw:x.observed.backingDeficitRaw})),
