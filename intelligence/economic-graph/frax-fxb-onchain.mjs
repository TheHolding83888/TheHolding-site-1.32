#!/usr/bin/env node
/**
 * The Holding · Frax FXB origin-series exact-block sensor v0.1
 *
 * Measures every currently source-bound FXB origin contract from the config
 * registry across Ethereum and Fraxtal. Bridged mirrors are topology only.
 *
 * Proven current state per origin series:
 * - FXB contract identity via FRAX() and configured Legacy Frax Dollar address;
 ...
 *
 * Spot price / auction price / implied yield are deliberately not invented here.
 * They remain a separate term-curve atom. UNKNOWN is never zero.
 */
...
export const FRAX_FXB_ONCHAIN_VERSION='0.1-frax-fxb-origin-series-exact-block';
...
function ratio18(n,d){if(BigInt(d)===0n)return null;const scaled=BigInt(n)*E18/BigInt(d);return units18(scaled);}
...
function loadConfig(config){
  ...
  if(x?.version!=='0.1-frax-fxb-series-registry'||x?.semantics?.executionAuthority!=='none'||x?.semantics?.originOnlyBackingMeasurement!==true)throw new Error('FXB series registry identity drift');
...
function unknown({attempts,reason='UNKNOWN-fxb-origin-series-read-failed',config=null}={}){
  return {
...
async function collectNetwork({network,networkConfig,endpoints,series,fetchImpl}){
  ...
  for(const endpoint of endpoints){
    try{
      const signatures=['FRAX()','MATURITY_TIMESTAMP()','totalSupply()','totalFxbMinted()','totalFxbRedeemed()','isRedeemable()','balanceOf(address)'];
      ...
      const block=blockReq.get(101).result;if(!block||String(block.number||'').toLowerCase()!==blockTag.toLowerCase()||!/^0x[0-9a-f]{64}$/i.test(String(block.hash||'')))throw new Error('FXB exact block identity unavailable');
      ...
      let id=1000;const calls=[],meta=[];
      for(const s of series){const ids={};for(const name of ['FRAX()','MATURITY_TIMESTAMP()','totalSupply()','totalFxbMinted()','totalFxbRedeemed()','isRedeemable()']){ids[name]=id++;calls.push(call(ids[name],s.originAddress,selector(name)));}ids.backing=id++;calls.push(call(ids.backing,networkConfig.lFrax,selector('balanceOf(address)')+addrArg(s.originAddress)));meta.push({series:s,ids});}
      const rows=await batched(endpoint.url,calls,fetchImpl),out=[];
      for(const m of meta){
        const frax=addressWord(rows.get(m.ids['FRAX()']).result),maturity=word(rows.get(m.ids['MATURITY_TIMESTAMP()']).result),supply=word(rows.get(m.ids['totalSupply()']).result),minted=word(rows.get(m.ids['totalFxbMinted()']).result),redeemed=word(rows.get(m.ids['totalFxbRedeemed()']).result),redeemable=word(rows.get(m.ids['isRedeemable()']).result)!==0n,backing=word(rows.get(m.ids.backing).result);
        const identityOk=sameAddress(frax,networkConfig.lFrax),supplyIdentityOk=minted>=redeemed&&minted-redeemed===supply,redeemabilityIdentityOk=redeemable===(BigInt(blockTimestamp)>=maturity),backingDeficit=backing>=supply?0n:supply-backing;
        if(!identityOk)throw new Error(`FXB ${m.series.id} LFRAX identity mismatch ${frax}`);
        if(!supplyIdentityOk)throw new Error(`FXB ${m.series.id} minted-redeemed-supply identity mismatch`);
        if(!redeemabilityIdentityOk)throw new Error(`FXB ${m.series.id} redeemability timestamp identity mismatch`);
        out.push({
          id:m.series.id,label:m.series.label,originNetwork:network,originAddress:m.series.originAddress,bridgedMirrors:m.series.bridgedMirrors||[],documentedMaturityDate:m.series.documentedMaturityDate,
          observed:{fraxAddress:frax,maturityTimestamp:Number(maturity),maturityIso:new Date(Number(maturity)*1000).toISOString(),isRedeemable:redeemable,totalSupplyRaw:supply.toString(),totalSupplyLfraxFace:units18(supply),totalFxbMintedRaw:minted.toString(),totalFxbRedeemedRaw:redeemed.toString(),backingLfraxRaw:backing.toString(),backingLfrax:units18(backing),backingCoverageRatio:ratio18(backing,supply),backingDeficitRaw:backingDeficit.toString(),secondsToMaturity:Number(maturity)-blockTimestamp},
          ...
          epistemic:{originContractState:'MEASURED-exact-block',redemptionAssetIdentity:'MEASURED-exact-contract-call',mintRedeemSupplyIdentity:'DERIVED-MECHANICAL-exact-contract-accounting',originBackingBalance:'MEASURED-exact-block',spotPrice:'UNKNOWN-not-measured-by-this-atom',impliedYield:'UNKNOWN-no-measured-price',bridgedMirrorBacking:'UNKNOWN-topology-only'}
        });
      }
...
export async function collectFraxFxbOnchain({config=null,rpcRegistry=null,fetchImpl=fetch,endpointsOverride=null}={}){
  ...
  const groups={};for(const s of cfg.series){if(!s?.originNetwork||!s?.originAddress)continue;(groups[s.originNetwork] ||= []).push(s);}
  const results=[];for(const [network,series] of Object.entries(groups)){const networkConfig=cfg.networks?.[network];if(!networkConfig)return unknown({attempts:[{network,endpointId:null,error:'network config missing'}],reason:'UNKNOWN-fxb-series-network-config-missing',config:cfg});results.push(await collectNetwork({network,networkConfig,endpoints:endpoints[network]||[],series,fetchImpl}));}
  ...
  if(!full)return unknown({attempts,reason:'UNKNOWN-fxb-incomplete-origin-series-coverage',config:cfg});
  return {
    ...
    networks:Object.fromEntries(results.map(x=>[x.network,{chainId:x.chainId,blockNumber:x.blockNumber,blockHash:x.blockHash,blockTimestamp:x.blockTimestamp,observedAt:x.observedAt,rpcEndpointId:x.rpc.endpointId}])),series:measuredSeries,
    coverage:{configuredOriginSeriesCount:cfg.series.length,measuredOriginSeriesCount:measuredSeries.length,unresolvedDocumentedSeriesCount:(cfg.documentedUnresolvedSeries||[]).length,unresolvedDocumentedSeries:cfg.documentedUnresolvedSeries||[],fullConfiguredOriginSeriesCoverage:true,originNetworkCount:results.length,activeSeriesCount:measuredSeries.filter(x=>!x.observed.isRedeemable).length,maturedSeriesCount:measuredSeries.filter(x=>x.observed.isRedeemable).length,seriesWithBackingDeficit:measuredSeries.filter(x=>x.observed.backingDeficitRaw!=='0').map(x=>x.id)},
    rpc:{networks:Object.fromEntries(results.map(x=>[x.network,x.rpc])),failoverAttempts:attempts},
    provenance:{seriesRegistryVersion:cfg.version,officialSources:cfg.officialSources,contractFunctions:['FRAX()','MATURITY_TIMESTAMP()','totalSupply()','totalFxbMinted()','totalFxbRedeemed()','isRedeemable()','LFRAX.balanceOf(FXB)']},
    epistemic:{originSeriesState:'MEASURED-exact-block-multi-chain',originBacking:'MEASURED-exact-block-origin-only',mintRedeemSupplyIdentity:'DERIVED-MECHANICAL-totalFxbMinted-minus-totalFxbRedeemed-equals-totalSupply',redeemabilityIdentity:'DERIVED-MECHANICAL-block-timestamp-vs-maturity',spotPrice:'UNKNOWN-not-measured-by-this-atom',impliedYield:'UNKNOWN-no-measured-price',auctionState:'UNKNOWN-separate-atom',bridgedMirrorBacking:'UNKNOWN-topology-only-not-double-counted',documentedUnresolvedSeries:'UNKNOWN-no-official-contract-address',unknownIsZero:false,causalClaimAuthority:'none',executionAuthority:'none'}
  };
}
...
export function applyFraxFxbOnchainMeasurement({state,previousState,measurement}){
  ...
  fxb.mechanicalRelations=[
    {from:'FXB FRAX() + configured origin chain',to:'Legacy Frax Dollar redemption-asset identity',class:valid?'MEASURED-exact-block':'UNKNOWN'},
