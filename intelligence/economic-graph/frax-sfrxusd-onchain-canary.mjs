...
function makeFetch({shareAssets=105n*E18,allFail=false}={}){
  return async (url,options)=>{
    ...
    if(payload.length===1&&payload[0].method==='eth_blockNumber')return {
      ok:true,
      async json(){return [{jsonrpc:'2.0',id:1,result:'0x64'}];}
    };
...
function baseState(generatedAt='2026-08-28T12:00:00.000Z'){
  return {
    generatedAt,
    authority:{executionAuthority:'none',causalClaimAuthority:'none'},
    protocolLifecycle:{
      summary:{protocolCount:8},
      protocols:{'registry-frax-vefrax':{maturityStage:'shadow'}}
    },
    protocolSensors:{
      'registry-frax-vefrax':{
        latest:{observation:{
...
assert.equal(first.values.frxUsdSupply,1000);
assert.equal(first.values.sfrxUsdSupply,100);
assert.equal(first.values.sfrxUsdTotalAssets,105);
assert.equal(first.values.sharePriceFrxUsd,1.05);
assert.equal(first.epistemic.executionAuthority,'none');

const state1=baseState();
applyFraxEcosystemSensor({state:state1,previousState:null});
applyFraxSfrxUsdOnchainMeasurement({state:state1,previousState:null,measurement:first});
const obs1=state1.protocolEvidence['registry-frax-ecosystem'].latest.observation;
assert.equal(obs1.coverage.surfaceCount,9);
assert.equal(obs1.coverage.measuredSurfaceCount,2);
assert.equal(obs1.coverage.sourceBoundUnknownSurfaceCount,7);
assert.equal(obs1.surfaces.frxUsdSfrxUsd.measurementState,'MEASURED-current-onchain-partial');
assert.equal(obs1.surfaces.frxUsdSfrxUsd.measured.intervalEmbeddedYield.status,'warming-first-onchain-checkpoint');
assert.equal(obs1.surfaces.frxUsdSfrxUsd.measured.intervalEmbeddedYield.accepted,false);

const second={
  ...first,
  observedAt:'2024-01-01T00:10:00.000Z',
  blockNumber:101,
  blockTag:'0x65',
  blockHash:`0x${'cd'.repeat(32)}`,
  values:{...first.values,sfrxUsdTotalAssets:106,sharePriceFrxUsd:1.06},
  raw:{...first.raw,sfrxUsdTotalAssets:(106n*E18).toString()}
};
const previous=structuredClone(state1);
const state2=baseState('2026-08-28T12:10:00.000Z');
applyFraxEcosystemSensor({state:state2,previousState:previous});
applyFraxSfrxUsdOnchainMeasurement({state:state2,previousState:previous,measurement:second});
const obs2=state2.protocolEvidence['registry-frax-ecosystem'].latest.observation;
const interval=obs2.surfaces.frxUsdSfrxUsd.measured.intervalEmbeddedYield;
assert.equal(interval.status,'ok');
assert.equal(interval.accepted,true);
assert.ok(Math.abs(interval.embeddedYieldPct-0.952380952381)<1e-9);
assert.equal(interval.annualizedApyPct,null);
assert.match(interval.annualizationState,/NOT-CALCULATED/);
assert.equal(obs2.epistemic.sfrxUsdEmbeddedYield,'DERIVED-MECHANICAL');
assert.equal(obs2.authority.executionAuthority,'none');

const unavailable=await collectFraxSfrxUsdOnchain({registry:minimalRegistry(),fetchImpl:makeFetch({allFail:true})});
assert.match(unavailable.status,/^UNKNOWN/);
assert.equal(unavailable.measurementClass,'UNKNOWN');
assert.equal(unavailable.values.sharePriceFrxUsd,null);
const state3=baseState();
applyFraxEcosystemSensor({state:state3,previousState:null});
applyFraxSfrxUsdOnchainMeasurement({state:state3,previousState:null,measurement:unavailable});
const obs3=state3.protocolEvidence['registry-frax-ecosystem'].latest.observation;
assert.equal(obs3.coverage.measuredSurfaceCount,1);
assert.equal(obs3.coverage.sourceBoundUnknownSurfaceCount,8);
assert.equal(obs3.surfaces.frxUsdSfrxUsd.measured.values.sharePriceFrxUsd,null);
assert.equal(obs3.epistemic.sfrxUsdCurrentState,'UNKNOWN');

console.log('FRAX sfrxUSD ONCHAIN SENSOR CANARY PASS',{
  exactBlock:first.blockNumber,
  rpcEndpoint:first.rpc.endpointId,
  sharePrice:first.values.sharePriceFrxUsd,
  firstInterval:obs1.surfaces.frxUsdSfrxUsd.measured.intervalEmbeddedYield.status,
  secondIntervalYieldPct:interval.embeddedYieldPct,
  measuredSurfaces:obs2.coverage.measuredSurfaceCount,
...
// Imported Fraxlend canaries perform deterministic tests locally and fail-closed
