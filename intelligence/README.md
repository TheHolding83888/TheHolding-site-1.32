...
## v0.2.1 — historical correction ledger

The Memory Vault now carries an append-only correction ledger at `intelligence/memory-vault/corrections.json`.

The first annotation preserves the original v0.1 baseline record unchanged while correcting five historical fields that were normalized as `0` even though the canonical interpretation was `null` / unknown. The correction is validated against a later v0.2 record with the same `sourceCompositeHash`.

Reasoning policy: **original records remain immutable; correction annotations take interpretation precedence for the specific targeted fields.**

Current correction ledger SHA-256 (canonical stable-object hash): `0623f58291d65ec7417e436a313f106d2e6aa62e23e1be255ad28c09a4bf1d6b`.
