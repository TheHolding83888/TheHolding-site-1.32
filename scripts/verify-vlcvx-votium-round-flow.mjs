...
if(x.engineVersion!=='0.1-votium-v2-protocol-native-round-accounting')fail('Votium round-flow engine mismatch');
...
for(const round of x.completedRounds){
  ...
  if(round.epistemic?.causalAttribution!=='unresolved'||round.epistemic?.primaryDriver!==null)fail(`Round ${round.roundId} causal boundary weakened`);
}
if(x.epistemic?.usdValuation!=='unknown-in-v0.1'||x.epistemic?.companyIncomeConnection!=='not-attributed-by-this-layer'||x.epistemic?.primaryDriver!==null)fail('Votium epistemic boundary weakened');
if(x.semantics?.unknownIsNotZero!==true||x.semantics?.contractAccountingIsNotUsdValuation!==true||x.semantics?.incentiveAndVoteCoexistenceIsNotCausation!==true||x.semantics?.protocolRoundFlowIsNotRealisedCompanyIncome!==true)fail('Votium semantic invariants missing');
