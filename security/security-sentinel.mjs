...
const SENTINEL_VERSION = '0.1-security-sentinel';
...
function short(v,n=12) { return typeof v === 'string' ? v.slice(0,n) : null; }
...
function scanWebCode(file,text) {
  const rp=rel(file);
  const checks=[
    ['dom-innerhtml','medium',/\.innerHTML\s*=\s*/g,'Dynamic innerHTML assignment detected.','If any assigned value later becomes user-controlled or external data, this can become a DOM-XSS sink.'],
  ...
  for (const m of text.matchAll(scriptRx)) {
    const tag=m[0];
    if (!/\bintegrity=["']/i.test(tag)) addFinding({severity:'medium',category:'external-script-no-sri',file:rp,line:lineOf(text,m.index),entity:m[1],summary:`External script without Subresource Integrity: ${m[1]}`,whyItMatters:'A compromised third-party script host can execute code in visitors’ browsers. SRI is useful when the asset is immutable.'});
  }
}
...
async function probe(url) {
  ...
  try {
    const res=await fetch(url,{redirect:'follow',signal:controller.signal,headers:{'user-agent':'The-Holding-Security-Sentinel/0.1'}});
    const headers={
...
const scanSnapshot={repository:{filesScanned:files.length,bytesScanned:scannedBytes,criticalHashes},site:{baseUrl:SITE_BASE,networkSkipped:SKIP_NETWORK,probes:siteProbes},findings};
...
if (findings.some(f=>f.category==='dom-innerhtml'||f.category==='dom-insert-html')) protectNext.push({priority:'medium',action:'Keep external/user-controlled data out of HTML execution sinks; prefer textContent for Observer/Brain surfaces.'});
...
const latest={version:LAYER_VERSION,sentinelVersion:SENTINEL_VERSION,generatedAt,status,headline,severityCounts,source:{filesScanned:files.length,bytesScanned:scannedBytes,siteProbes:siteProbes.length},whatChanged:{newFindings:newFindings.slice(0,30),resolvedFindings:resolvedFindings.slice(0,30)},currentFindings:findings.slice(0,100),protectNext,bridge:{purpose:'Compact security handoff for The Holding Brain / human review.',snapshotHash,securityVault:{runCount:manifest.runCount,startedAt:manifest.startedAt,latestRecordPath:recordPath,latestRecordHash:recordHash,chainRootHash:manifest.chainRootHash}}};

function buildBrief(){const l=[];l.push('# The Holding — Autonomous Security Intelligence','',`**Generated:** ${generatedAt}`,`**Sentinel:** ${SENTINEL_VERSION}`,'',`## ${headline}`,'',`- Critical: ${severityCounts.critical}` ,`- High: ${severityCounts.high}`,`- Medium: ${severityCounts.medium}`,`- New findings: ${newFindings.length}`,`- Resolved findings: ${resolvedFindings.length}`,'','## Protect next','');for(const p of protectNext)l.push(`- **${p.priority}** — ${p.action}`);l.push('','## Permanent security memory','',`- Security Vault runs: ${manifest.runCount}.`,`- Latest record: \`${recordPath}\`.`,'- Vault retention: indefinite / append-only hash chain.','','---','The Sentinel does not expose matched secrets in its reports and does not make autonomous destructive changes. Critical findings are intended to fail the workflow after the safe report is published.','');return l.join('\n');}
...
console.log(`Status: ${status}`); console.log(`Findings: ${findings.length} | critical ${severityCounts.critical} | high ${severityCounts.high}`); console.log(`New: ${newFindings.length} | resolved ${resolvedFindings.length}`); console.log(`Security Vault runs: ${manifest.runCount}`); console.log(`Snapshot: ${snapshotHash}`);
