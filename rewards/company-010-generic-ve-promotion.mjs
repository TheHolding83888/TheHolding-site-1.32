...
function addStateNativeFirstOnboarding(source){
  ...
  const replacement=[
    ...
    '    }',
    '',
    '    let recent = firstOnboarding?.complete === true ? {',
    '      complete: true,',
    ...
    '    }',
    '',
    '    // A persisted bootstrap-complete reward index is authoritative only through',
    '    // its checkpoint. If the cheap incremental Voted tail is unavailable because',
    '    // public RPC / explorer transport fails, recover completeness from protocol',
    '    // state instead of accepting a blind gap: enumerate the complete current Voter',
    '    // registry and re-prove this tokenId participation on every reward contract.',
    '    // This is slower but fail-closed and runs only as a bounded recovery path.',
    '    if (indexEntry && indexEntry.bootstrapComplete === true && recent.complete !== true) {',
    '      try {',
    '        const registryRecovery = await currentVoterRewardRegistry(cfg, provider);',
    '        if (registryRecovery.complete === true) {',
    '          const recoveryRows = await mapLimit(registryRecovery.entries, 24, async entry => ({',
    '            entry,',
    '            participation: await probeRewardParticipation(entry.rewardAddress, tokenId, provider, cfg)',
    '          }));',
    '          const recoveryUnknown = recoveryRows.filter(x => x.participation?.known !== true);',
    '          if (recoveryUnknown.length === 0) {',
    '            const recoveryParticipated = recoveryRows',
    '              .filter(x => x.participation?.participates === true)',
    '              .map(x => ({',
    '                ...x.entry,',
    '                firstSeenAt: indexEntry?.contracts?.find(y => String(y.rewardAddress || "").toLowerCase() === String(x.entry.rewardAddress || "").toLowerCase())?.firstSeenAt || NOW,',
    '                lastSeenAt: NOW',
    '              }));',
    '            const recoveryBlock = Number(await provider.getBlockNumber());',
    '            const priorErrors = [...(recent.errors || [])];',
    '            indexEntry = upsertDirectVeIndex(cfg.providerKey, tokenId, {',
    '              contracts: recoveryParticipated,',
    '              observedAt: NOW,',
    '              recentStartBlock: recent.startBlock,',
    '              recentThroughBlock: recoveryBlock,',
    "              bootstrapSource: indexEntry.bootstrapSource || 'current-voter-registry-participation',",
    '              bootstrapComplete: true,',
    '              lastUpdatedAt: NOW',
    '            });',
    '            recent = {',
    '              complete: true,',
    "              mode: recent.mode + ':state-registry-recovery',",
    "              source: 'onchain-current-voter-registry-recovery',",
    '              startBlock: recent.startBlock,',
    '              throughBlock: recoveryBlock,',
    '              latestBlock: recoveryBlock,',
    '              pools: [],',
    '              logCount: recent.logCount || 0,',
    '              requestCount: (recent.requestCount || 0) + recoveryRows.length,',
    '              budgetMs: recent.budgetMs,',
    "              errors: [...priorErrors, 'tail transport recovered by complete Voter registry participation proof']",
    '            };',
    '          } else {',
    "            recent.errors = [...(recent.errors || []), 'registry-recovery-participation-unknown:' + recoveryUnknown.length];",
    '          }',
    '        } else {',
    "          recent.errors = [...(recent.errors || []), 'registry-recovery-incomplete:' + (registryRecovery.issues || []).join(' | ')];",
    '        }',
    '      } catch (e) {',
    "        recent.errors = [...(recent.errors || []), 'registry-recovery:' + redactKnownRpcUrls(e?.message || e)];",
    '      }',
    '    }'
  ].join('\n');
