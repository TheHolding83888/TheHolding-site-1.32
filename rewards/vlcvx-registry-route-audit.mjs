...
const VERSION = '0.2-vlcvx-full-registry-route-audit';
...
const CONVEX_FINANCE_DELEGATE = getAddress('0x947B7742C403f20e5FaCcDAc5E092C943E7D0277');
...
function delegateIdentity(addr,votiumEns){
  ...
  if(same(addr,CONVEX_FINANCE_DELEGATE))return{kind:'convex-finance',label:'Convex Finance'};
...
function classify(delegate,forward,effective){
  ...
  if(delegate.kind==='convex-finance')return{routeId:'convex-finance-vlcvx',publicLabel:'Convex Finance · vlCVX',path:'vlCVX → Convex Finance'};
...
async function main(){
  ...
  for(const company of COMPANIES){
    ...
    for(const w of company.wallets){
      const resolved=w.address||await p.resolveName(w.ens);if(!resolved)throw new Error(`wallet unresolved: ${w.alias}`);const address=getAddress(resolved);
      ...
      const balance=fmt(bal),lockedAmount=fmt(locked.locked),unlockable=fmt(locked.unlockable),total=fmt(locked.total),delegatedWeight=fmt(weight),hasVlCvx=balance>1e-12||lockedAmount>1e-12||total>1e-12;
      const d=delegateIdentity(delegate,votiumEns),f=forwardingIdentity(fwd.to),start=Number(fwd.start),expiration=Number(fwd.expiration),forwardEpochN=Number(forwardEpoch),forwardingEffective=start>0&&start<=forwardEpochN&&expiration>forwardEpochN,route=hasVlCvx?classify(d,f,forwardingEffective):null;
    ...
    const positive=walletRows.filter(x=>x.hasVlCvx);companies.push({registry:company.registry,name:company.name,hasVlCvx:positive.length>0,totalVlCvx:positive.reduce((s,x)=>s+x.vlCvx.balance,0),routes:[...new Set(positive.map(x=>x.route?.publicLabel).filter(Boolean))],wallets:walletRows});
  }
  const positive=companies.filter(x=>x.hasVlCvx);
  const out={version:VERSION,generatedAt,executionAuthority:'none',methodology:{principal:'Convex vlCVX locker balanceOf + lockedBalances',delegation:'current Convex onchain GaugeDelegation.getDelegateAtEpoch at newest usable Curve GaugeVotePlatform vlCVX epoch',settlement:'Votium Forwarder Registry registry(wallet) at registry currentEpoch',delegateIdentity:'Votium identity independently resolves through votium.eth; Stake DAO addresses are pinned from its current vlCVX distribution implementation; Convex Finance delegate is pinned from the current observed onchain delegate and independently identified as Convex Finance deployer/reward-manager infrastructure.',unknownIsNotZero:true,claimAuthority:'none'},contracts:{vlCVX:VLCVX,gaugeDelegation:GAUGE_DELEGATION,curveGaugeVotePlatform:CURVE_GAUGE_PLATFORM,votiumForwarderRegistry:FORWARD_REGISTRY,votiumDelegate:VOTIUM_DOCS_DELEGATE,votiumEnsResolved:votiumEns,stakeDaoDelegate:STAKEDAO_DELEGATE,convexFinanceDelegate:CONVEX_FINANCE_DELEGATE,unionForward:UNION_FORWARD,stakeDaoForward:STAKEDAO_FORWARD},epochs:{vlCvxVote:voteEpoch,votiumForwarding:Number(forwardEpoch)},summary:{companyCount:companies.length,vlCvxCompanyCount:positive.length,vlCvxCompanies:positive.map(x=>({registry:x.registry,name:x.name,totalVlCvx:x.totalVlCvx,routes:x.routes}))},companies};
