...
async function main(){
 ...
 const prior=readHistory();const prev=[...(prior?.checkpoints||[])].reverse().find(x=>x&&x.sharesRaw&&x.ppfsRaw)||null;
 ...
 const checkpoint={generatedAt:now,blockNumber,sharesRaw:sharesRaw.toString(),shares:round(shares,12),ppfsRaw:ppfsRaw.toString(),ppfs:round(ppfs,18),underlyingCvxCrv:round(underlying,12),cvxCrvPriceUsd:round(priceUsd,8),valueUsd:round(underlying*priceUsd,6)};
 const checkpoints=[...(prior?.checkpoints||[]),checkpoint].filter((x,i,a)=>i===a.findIndex(y=>y.generatedAt===x.generatedAt)).slice(-730);
 const history={version:'0.1-company-009-beefy-cvxcrv-embedded-income',generatedAt:now,company:{registry:'009',name:'1milliondollar.eth',address:COMPANY},strategy:{id:'beefy-cvxcrv',vaultId:'convex-staked-cvxCRV',vault:VAULT,want:CVXCRV,incomeMode:'compounded-embedded',claimableApplicable:false},latestCheckpoint:checkpoint,latestInterval:interval,checkpoints,methodology:{rule:'Beefy harvested Convex rewards are embedded in vault PPFS. A measured interval is admitted only when adjacent checkpoints have the exact same vault share balance; otherwise flow reconciliation is required.',noBackfill:'No historical income is inferred from current APY or from a single checkpoint.',claimableBoundary:'Embedded Beefy income is never added to Accrued Rewards / Claimable.'},authority:{executionAuthority:'none',transactions:false,claimingAuthority:false}};
 ...
 if(fs.existsSync(REWARDS)){
   ...
   c.embeddedIncome=(c.embeddedIncome||[]).filter(x=>x.route!=='beefy-cvxcrv');
   ...
   c.measuredEmbeddedUsd=round(embedded,6);c.measuredEarnedUsd=round(claimable+embedded,6);c.measuredEarnedUsdIsComplete=false;c.embeddedIncomeStatus=interval.status;c.updatedAt=now;
   d.diagnostics=d.diagnostics||{};d.diagnostics.company009BeefyCvxCrv={version:history.version,generatedAt:now,status:interval.status,checkpointCount:checkpoints.length,executionAuthority:'none'};
   fs.writeFileSync(REWARDS,JSON.stringify(d,null,2)+'\n');
 }
 console.log('COMPANY #009 BEEFY cvxCRV INCOME PASS',JSON.stringify({status:interval.status,shares:checkpoint.shares,ppfs:checkpoint.ppfs,underlying:checkpoint.underlyingCvxCrv,incomeCvxCrv:interval.incomeCvxCrv,incomeUsd:interval.incomeUsd,checkpointCount:checkpoints.length,executionAuthority:'none'},null,2));
}
