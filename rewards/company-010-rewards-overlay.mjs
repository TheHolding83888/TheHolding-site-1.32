...
function veRoute(key,row,token){
  ...
  const price=finite(row?.priceUsd)?Number(row.priceUsd):null;
  ...
  const embedded=finite(x.compounded?.amount)&&Number(x.compounded.amount)>0?{protocol:x.protocol,route:x.route,state:'Compounded',claimableApplicable:false,symbol:x.compounded.symbol,amount:round(x.compounded.amount,12),classification:'compounded-locked',metric:'LockedManagedReward.earned(baseToken, veNFT)',note:'Managed veNFT reward remains locked in the strategy and is represented as Compounded, not as freely claimable income.'}:null;
