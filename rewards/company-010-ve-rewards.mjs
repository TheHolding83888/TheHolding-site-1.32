...
async function collect(cfg){
  ...
  const publicState=mode==='managed-compounded'?'Compounded':'Pending';
...
const out={version:'0.1-company-010-ve-reward-state',generatedAt:new Date().toISOString(),company:'Cypher',walletScope:{alias:'Wallet 2',address:wallet2.address},routes,semantics:{managedLockedReward:'Compounded',directRoute:'Pending until the established full direct reward index is bound for Company #010',unknown:'Pending',unknownIsNotZero:true},authority:{readOnly:true,transactions:false,executionAuthority:'none'}};
