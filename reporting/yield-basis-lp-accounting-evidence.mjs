#!/usr/bin/env node
/**
 * The Holding · Yield Basis LP factual tracking evidence v0.2
 *
...
export const VERSION='0.2-yield-basis-lp-custody-aware-factual-tracking';
...
async function rpc(url,method,params=[],timeoutMs=TIMEOUT_MS){
  ...
  try{
    const response=await fetch(url,{method:'POST',signal:controller.signal,cache:'no-store',headers:{'content-type':'application/json','accept':'application/json','user-agent':'The-Holding-Yield-Basis-LP-Accounting/0.2'},body:JSON.stringify({jsonrpc:'2.0',id:1,method,params})});
...
function validateCanonicalMarketBindings(discovery){
  const positions=Array.isArray(discovery?.discovery?.yieldBasis?.positions)?discovery.discovery.yieldBasis.positions:[];
  for(const expected of Object.values(MARKETS)){
    const row=positions.find(x=>x?.market===expected.market);
    if(!row||lower(row.lt)!==expected.lt||lower(row.gauge)!==expected.gauge)throw new Error(`${expected.market} canonical LT/gauge binding mismatch`);
  }
  return true;
}
...
function rowHasHolding(row){
  return (Array.isArray(row?.holdings)?row.holdings:[]).some(x=>positiveHex(x?.directLtBalanceRaw)||positiveHex(x?.gaugeShareBalanceRaw));
}
...
export function validateEvidenceOutput(output){
  ...
  for(const row of checkpoints){
    ...
    if(!expected||row?.company!==COMPANY||String(row?.registry)!==REGISTRY||row?.chain!=='Ethereum'||row?.protocol!=='Yield Basis'||row?.market!==expected.market||lower(row?.lt)!==expected.lt||lower(row?.gauge)!==expected.gauge)throw new Error('Yield Basis LP checkpoint identity mismatch');
    ...
    if(!holdings.length||holdings.some(x=>!isAddress(x?.wallet)||!isHexQuantity(x?.directLtBalanceRaw)||!isHexQuantity(x?.gaugeShareBalanceRaw))||!rowHasHolding(row))throw new Error(`${row?.engineId} custody-aware holding proof invalid`);
    if(!['direct-lt','gauge','mixed'].includes(row?.activeHoldingPath))throw new Error(`${row?.engineId} active holding path invalid`);
  }
  return true;
}
...
export async function buildEvidence({productivity,discovery,rpcUrl,blockHex}={}){
  ...
  validateCanonicalMarketBindings(discovery);
  ...
  for(const[engineId,market]of Object.entries(MARKETS)){
    ...
    for(const wallet of wallets){
      ...
      const directLtBalanceRaw=await rpc(rpcUrl,'eth_call',[{to:market.lt,data},blockHex]);
      const gaugeShareBalanceRaw=await rpc(rpcUrl,'eth_call',[{to:market.gauge,data},blockHex]);
      if(!isHexQuantity(directLtBalanceRaw)||!isHexQuantity(gaugeShareBalanceRaw))throw new Error(`${engineId} malformed custody balance result`);
      holdings.push({wallet,directLtBalanceRaw,gaugeShareBalanceRaw});
    }
    const directCount=holdings.filter(x=>positiveHex(x.directLtBalanceRaw)).length;
    const gaugeCount=holdings.filter(x=>positiveHex(x.gaugeShareBalanceRaw)).length;
    if(!directCount&&!gaugeCount)throw new Error(`${engineId} canonical wallets hold no direct LT or canonical gauge shares`);
    const activeHoldingPath=directCount&&gaugeCount?'mixed':gaugeCount?'gauge':'direct-lt';
    checkpoints.push({ok:true,engineId,company:COMPANY,registry:REGISTRY,chain:'Ethereum',protocol:'Yield Basis',market:market.market,family:market.family,lt:market.lt,gauge:market.gauge,blockNumber,observedAt,source:'ethereum-json-rpc',sourceMethod:'LT.pricePerShare() + LT.balanceOf(company wallet) + canonical gauge.balanceOf(company wallet), same block',rpcHost:safeHost(rpcUrl),pricePerShareRaw:pps,holdings,positiveDirectLtHolderCount:directCount,positiveGaugeHolderCount:gaugeCount,activeHoldingPath,factualTrackingProof:true,periodIncomeAuthority:false,openingBalanceCreatesIncome:false,referenceAprUsed:false,unknownIsNotZero:true,executionAuthority:'none',checkpointKey:`yield-basis-lp:${engineId}:${blockNumber}`});
  }
