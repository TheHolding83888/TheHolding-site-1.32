...
export function annotateFortyAcresSettlementRecognition(ledger,defiteaSource){
  ...
  if(contract?.version!==FORTY_ACRES_SETTLEMENT_VERSION||contract?.route!==FORTY_ACRES_ROUTE||contract?.principalId!=='velodrome-finance'||contract?.referenceDoubleCountPrevented!==true||contract?.unknownIsNotZero!==true||contract?.executionAuthority!=='none')throw new Error('40 Acres settlement recognition contract unavailable or drifted');
  ...
  const events=(ledger?.events||[]).map(event=>{
    ...
    if(event.company!=='defitea.eth'||event.family!=='realised-cash-flow'||event.route!==FORTY_ACRES_ROUTE||event.protocol!=='40 Acres · veVELO'||event.sourceFile!=='reporting/defitea-income-ledger.json'||event.sourceFamily!=='fortyAcresReceivedEvents'||event.evidenceStatus!=='canonical-actual-net-received'||!event.physicalEventId||String(event.economicDate||'')<trackingSince||event.executionAuthority!=='none')throw new Error(`40 Acres canonical settlement identity drift: ${event?.eventKey||'unknown'}`);
    const prior=event.incomeRecognition||null;
    if(prior&&(prior.recognizesEarnedIncome!==false||prior.settlementOf!==FORTY_ACRES_SETTLEMENT_OF))throw new Error(`40 Acres income-recognition conflict: ${event.eventKey}`);
    annotated++;
    return{...event,incomeRecognition:{recognizesEarnedIncome:false,settlementOf:FORTY_ACRES_SETTLEMENT_OF,recognitionBasis:'settlement-of-canonical-ve33-earned-accrual-lane',settlementStatus:'settled-net-receipt',mechanismId:'velodrome_vevelo',sourceContractVersion:contract.version,economicFieldsMutated:false,executionAuthority:'none'}};
  });
  if(annotated!==sourceKeys.size)throw new Error(`40 Acres settlement recognition parity mismatch: ${annotated}/${sourceKeys.size}`);
  return{ledger:{...ledger,events,sourceState:{...(ledger?.sourceState||{}),fortyAcresSettlementRecognition:{source:'reporting/defitea-income-ledger.json#fortyAcresSettlement',version:'0.1-canonical-settlement-only-recognition',sourceContractVersion:contract.version,route:FORTY_ACRES_ROUTE,mechanismId:'velodrome_vevelo',settlementOf:FORTY_ACRES_SETTLEMENT_OF,eventCount:annotated,economicFieldsMutated:false,referenceAprUsed:false,executionAuthority:'none'}},accountingExtensions:{...(ledger?.accountingExtensions||{}),fortyAcresSettlementRecognition:{version:'0.1-canonical-settlement-only-recognition',family:'realised-cash-flow',recognizesEarnedIncome:false,settlementOf:FORTY_ACRES_SETTLEMENT_OF,accruedIncomeRemainsEarnedIncomeAuthority:true,receiptDoesNotReRecognizeIncome:true,unknownIsNotZero:true,executionAuthority:'none'}}},annotated};
}

function withoutValuationResolution(event){if(!event?.valuationResolution)return event;const{valuationResolution,...clean}=event;return clean;}
function reusableIdentityBoundResolution(event,identity){
  const r=event?.valuationResolution||null;
  if(r?.version!==HISTORICAL_VALUATION_RESOLUTION_VERSION||r?.identityBound!==true||String(r?.identityToken||'').toLowerCase()!==identity.token||String(r?.boundaryAt||'')!==String(event?.periodEnd||'')||r?.economicFieldsMutated!==false||r?.referenceAprUsed!==false||r?.currentPriceUsed!==false||r?.unknownIsNotZero!==true||r?.executionAuthority!=='none'||r?.originalUsdValue!==null||!finite(r?.valuationUnitUsd)||Number(r.valuationUnitUsd)<=0||!finite(r?.resolvedUsdValue)||Number(r.resolvedUsdValue)<=0||!historicalValuationSourceMatchesVe33Identity(event,r))return false;
...
export async function annotateHistoricalValuationResolution(ledger,{resolver=historicalCanonicalPriceAtBoundary}={}){
  ...
  for(const event of ledger?.events||[]){
    ...
    if(!identity.ok){unresolved++;unresolvedStatuses[identity.status]=(unresolvedStatuses[identity.status]||0)+1;if(prior)invalidPriorResolutionClearedCount++;events.push(cleanEvent);continue;}
    if(identity.eventTokenMatchesIdentity!==true)identityMismatchEventCount++;
    if(reusableIdentityBoundResolution(event,identity)){resolved++;reusedIdentityBoundResolutionCount++;events.push(event);continue;}
    const key=`${identity.token}|${boundaryAt}|${identity.closeBlock}`;
    if(!cache.has(key))cache.set(key,Promise.resolve().then(()=>resolver({token:identity.token,boundaryAt,eventKey:event.eventKey,sourceIdentity:event.sourceIdentity})).catch(error=>({ok:false,status:'historical-canonical-price-resolver-error',error:error?.message||String(error)})));
    ...
    if(valuation?.ok!==true||!(Number.isFinite(price)&&price>0)||!Number.isFinite(observedMs)||observedMs>boundaryMs){unresolved++;const status=valuation?.status||'historical-canonical-price-unavailable';unresolvedStatuses[status]=(unresolvedStatuses[status]||0)+1;if(prior)invalidPriorResolutionClearedCount++;events.push(cleanEvent);continue;}
  ...
  return{ledger:{...ledger,events,sourceState:{...(ledger?.sourceState||{}),historicalValuationResolution:{...summary,source:'canonical Market Data Git history, exact historical onchain Chainlink, explicit or factory-discovered exact-block Velodrome TWAP plus Chainlink quote, or explicit or factory-discovered exact-block Aerodrome/Slipstream TWAP plus Chainlink quote at original accounting boundary'}},accountingExtensions:{...(ledger?.accountingExtensions||{}),historicalValuationResolution:{...summary,immutableEventUsdValueRewritten:false,resolvedValueLivesInNonEconomicMetadata:true,mutableEventTokenMetadataIsNotValuationAuthority:true,stablecoinPegAssumptionUsed:false}}},...summary};
}

function annotateProjectX(rebuilt,history,built,newEventsAdmitted,integrity,generatedAt){
  const s=built.summary;
  return{...rebuilt,generatedAt,sourceState:{...(rebuilt.sourceState||{}),projectXFeeAccrual:{file:'companies/company-010-projectx-rate-history.json',version:history?.version||null,engineVersion:history?.engineVersion||null,generatedAt:history?.generatedAt||null,observationCount:s.observationCount,intervalCount:s.intervalCount,acceptedIntervalCount:s.acceptedIntervalCount,measuredZeroIntervalCount:s.measuredZeroIntervalCount,fingerprintBoundaryCount:s.fingerprintBoundaryCount,claimResetBoundaryCount:s.claimResetBoundaryCount,crossMonthBoundaryCount:s.crossMonthBoundaryCount,candidateEventCount:s.candidateEventCount,sourceMethod:'adjacent same-fingerprint collect.staticCall fee growth',referenceAprUsed:false,currentClaimableBalanceIsPeriodIncome:false,laterPriceMovementRewritesClosedIncome:false,coreGitBlob:integrity.coreSha,candidateModuleGitBlob:integrity.candidateSha}},run:{...(rebuilt.run||{}),projectXCandidateEventCount:s.candidateEventCount,projectXNewEventsAdmitted:newEventsAdmitted,projectXObservationCount:s.observationCount,projectXAcceptedIntervalCount:s.acceptedIntervalCount,projectXBoundaryCount:s.fingerprintBoundaryCount+s.claimResetBoundaryCount+s.crossMonthBoundaryCount},accountingExtensions:{...(rebuilt.accountingExtensions||{}),projectXFeeAccrual:{version:built.version,source:'companies/company-010-projectx-rate-history.json',company:'Cypher',route:'projectx-whype-usdc',family:'accrued-entitlement',openingBalanceCreatesIncome:false,fingerprintChangeCreatesIncome:false,claimOrResetCreatesIncome:false,claimOrResetIsSettlementBoundary:true,crossMonthIntervalAutoAllocated:false,currentClaimableBalanceIsPeriodIncome:false,endpointObservedPriceFreezesValuation:true,referenceAprUsed:false,laterPriceMovementRewritesClosedIncome:false,unknownIsNotZero:true,executionAuthority:'none'}}};
}

export async function build(){
  const integrity=await verifyExtensionIntegrity();const base=await core.build();
  const[history,defiteaSource]=await Promise.all([readJson(HISTORY_FILE),readJson(DEFITEA_LEDGER_FILE)]);
  const generatedAt=base?.generatedAt||new Date().toISOString();const built=buildProjectXIncomeCandidates(history,core.finalizeCandidate,generatedAt);const admitted=core.admitEvents(base?.events,built.candidates);let rebuilt=base;
  if(admitted.admitted>0){await writeJson(OUTPUT_FILE,{...base,events:admitted.events});rebuilt=await core.build();}
  const projectXAnnotated=annotateProjectX(rebuilt,history,built,admitted.admitted,integrity,generatedAt);const settlementAnnotated=annotateFortyAcresSettlementRecognition(projectXAnnotated,defiteaSource);const valuationAnnotated=await annotateHistoricalValuationResolution(settlementAnnotated.ledger);
  return{...valuationAnnotated.ledger,run:{...(valuationAnnotated.ledger.run||{}),fortyAcresSettlementOnlyRecognitionCount:settlementAnnotated.annotated,historicalValuationResolvedEventCount:valuationAnnotated.resolvedEventCount,historicalValuationUnresolvedEventCount:valuationAnnotated.unresolvedEventCount}};
}

async function main(){
  const output=await build();await writeJson(OUTPUT_FILE,output);
  console.log('Canonical Income Ledger built with factual accrual, settlement recognition and historical valuation resolution',{events:output.events?.length||0,newEvents:output.run?.newEventsAdmitted||0,projectXCandidates:output.run?.projectXCandidateEventCount||0,projectXNewEvents:output.run?.projectXNewEventsAdmitted||0,projectXAcceptedIntervals:output.run?.projectXAcceptedIntervalCount||0,fortyAcresSettlementOnlyRecognitions:output.run?.fortyAcresSettlementOnlyRecognitionCount||0,historicalValuationResolvedEvents:output.run?.historicalValuationResolvedEventCount||0,historicalValuationUnresolvedEvents:output.run?.historicalValuationUnresolvedEventCount||0,claimableSnapshots:output.claimableSnapshots?.length||0,companies:Object.keys(output.companies||{}).length,unknownIsNotZero:output.semantics?.unknownIsNotZero===true,referenceAprUsed:false,executionAuthority:output.authority?.executionAuthority||null});
}
