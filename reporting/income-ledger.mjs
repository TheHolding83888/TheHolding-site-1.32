...
const MAX_EXACT_BLOCK_BOUNDARY_LAG_MS=120_000;
...
function reusableIdentityBoundResolution(event,identity){
  ...
  if(EXACT_BLOCK_SOURCE_FAMILIES.has(r.sourceFamily)){
    const pegMetadataValid=r.sourceFamily==='historical-onchain-chainlink-at-boundary'?r?.stablecoinPegAssumptionUsed!==true:r?.stablecoinPegAssumptionUsed===false;
    ...
    if(!Number.isFinite(blockMs)||blockMs>boundaryMs||boundaryMs-blockMs>MAX_EXACT_BLOCK_BOUNDARY_LAG_MS||observedMs>blockMs)return false;
  }
...
export async function annotateHistoricalValuationResolution(ledger,{resolver=historicalCanonicalPriceAtBoundary}={}){
  ...
  for(const event of ledger?.events||[]){
    const boundaryAt=String(event?.periodEnd||''),boundaryMs=Date.parse(boundaryAt);
    const isEligible=event?.family==='accrued-entitlement'&&event?.sourceFile===VE33_SOURCE_FILE&&event?.usdValue===null&&event?.valuationStatus==='unvalued-fail-closed'&&Number.isFinite(boundaryMs)&&finite(event?.amount)&&Number(event.amount)>0&&event?.unknownIsNotZero===true&&event?.executionAuthority==='none';
    ...
    const key=`${Number(event?.chainId)||0}|${identity.token}|${boundaryAt}|${identity.closeBlock}`;
    if(!cache.has(key))cache.set(key,Promise.resolve().then(()=>resolver({token:identity.token,boundaryAt,eventKey:event.eventKey,sourceIdentity:event.sourceIdentity,chainId:event?.chainId})).catch(error=>({ok:false,status:'historical-canonical-price-resolver-error',error:error?.message||String(error)})));
    const valuation=await cache.get(key);
    const price=Number(valuation?.priceUsd),observedMs=Date.parse(valuation?.observedAt||'');
    ...
    if(EXACT_BLOCK_SOURCE_FAMILIES.has(sourceFamily)){
      const blockMs=Date.parse(resolution.sourceBlockTimestamp||'');
      if(Number(resolution.sourceBlockNumber)!==Number(identity.closeBlock)||!Number.isFinite(blockMs)||blockMs>boundaryMs||boundaryMs-blockMs>MAX_EXACT_BLOCK_BOUNDARY_LAG_MS){unresolved++;unresolvedStatuses['historical-onchain-block-identity-mismatch']=(unresolvedStatuses['historical-onchain-block-identity-mismatch']||0)+1;if(prior)invalidPriorResolutionClearedCount++;events.push(cleanEvent);continue;}
    }
