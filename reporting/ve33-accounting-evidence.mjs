...
function priceMap(rewards){
  ...
  for(const c of Object.values(rewards?.companies||{}))for(const r of c?.rewards||[]){
    ...
    const k=lower(r.token),arr=map.get(k)||[];arr.push({priceUsd:Number(r.priceUsd),priceMethod:r.priceMethod||null,observedAt:rewards.generatedAt||null});map.set(k,arr);
  }
  const out=new Map();
...
function sourcePositions(source){
  ...
  const seen=new Set();
  return out.filter(p=>{const k=`${lower(p.holder)}:${p.tokenId}:${p.mode||''}:${lower(p.freeManagedReward||'')}`;if(seen.has(k))return false;seen.add(k);return true;});
}
    ...
    for(const source of c?.sources||[]){
      for(const[protocolKey,cfg]of Object.entries(PROTOCOLS)){
        ...
        for(const p of sourcePositions(source)){
          rows.push({company,protocolKey,protocol:cfg.protocol,route:source.route,holder:p.holder,walletAlias:p.walletAlias||null,tokenId:String(p.tokenId),mode:p.mode||null,custodyContext:p.custodyContext||'direct-wallet',managedTokenId:p.managedTokenId?String(p.managedTokenId):null,freeManagedReward:isAddress(p.freeManagedReward)?getAddress(p.freeManagedReward):null,rewardContracts:currentRewardContracts(rewards,company,source.route,p.tokenId,cfg.providerKey)});
        }
      }
    }
  }
  const seen=new Set();
  return rows.filter(x=>{const k=[x.company,x.protocolKey,x.route,lower(x.holder),x.tokenId,x.mode].join('|');if(seen.has(k))return false;seen.add(k);return true;});
}

async function providerFor(cfg){
  const urls=unique([process.env[cfg.rpcEnv],...cfg.rpcFallbacks]);let last=null;
  for(const url of urls){try{const p=new JsonRpcProvider(url,cfg.chainId);await Promise.race([p.getBlockNumber(),new Promise((_,rej)=>setTimeout(()=>rej(new Error('RPC timeout')),8000))]);return p;}catch(e){last=e;}}
...
async function blockAtOrBefore(provider,timestamp,latestNumber,cache){
  ...
  const target=Math.floor(Date.parse(timestamp)/1000);if(!Number.isFinite(target))throw new Error(`Invalid timestamp ${timestamp}`);
  let lo=0,hi=latestNumber;
  while(lo<hi){const mid=Math.ceil((lo+hi)/2),b=await provider.getBlock(mid);if(!b)throw new Error(`Block ${mid} unavailable`);if(Number(b.timestamp)<=target)lo=mid;else hi=mid-1;}
  const b=await provider.getBlock(lo);if(!b)throw new Error(`Boundary block ${lo} unavailable`);
  const out={blockNumber:Number(b.number),blockTimestamp:new Date(Number(b.timestamp)*1000).toISOString()};cache.set(key,out);return out;
}
...
async function rewardTokens(contract,blockTag){
  ...
  for(let i=0;i<n;i++){const token=getAddress(await contract.rewards(i,{blockTag}));if(!out.some(x=>lower(x)===lower(token)))out.push(token);}
  return out;
}

async function tokenMeta(provider,token,blockTag){
  const c=new Contract(token,ERC20_ABI,provider);let symbol='TOKEN',decimals=18;
  try{symbol=await c.symbol({blockTag});}catch{}try{decimals=Number(await c.decimals({blockTag}));}catch{}
...
export function decodeRewardClaimTokenId({to,data,rewardContract,rewardToken,voter}){
  try{
    if(lower(to)===lower(rewardContract)){
      const p=DIRECT_REWARD_IFACE.parseTransaction({data});if(!p||p.name!=='getReward')return null;
      const tokens=[...p.args[1]].map(lower);if(!tokens.includes(lower(rewardToken)))return null;return String(p.args[0]);
    }
    if(lower(to)===lower(voter)){
      const p=VOTER_CLAIM_IFACE.parseTransaction({data});if(!p||!['claimBribes','claimFees'].includes(p.name))return null;
      const contracts=[...p.args[0]],tokens=[...p.args[1]],idx=contracts.findIndex(x=>lower(x)===lower(rewardContract));
      if(idx<0||!Array.from(tokens[idx]||[]).map(lower).includes(lower(rewardToken)))return null;return String(p.args[2]);
    }
  }catch{}
  return null;
}
...
async function rewardClaimSettlements({provider,contract,cfg,lane,fromBlock,toBlock}){
  ...
  const filter=contract.filters.ClaimRewards(null,lane.rewardToken),events=[],unresolved=[];let total=0n;
  for(let from=fromBlock;from<=toBlock;from+=MAX_LOG_BLOCKS){
    ...
    for(const log of logs){
      const recipient=String(log?.args?.[0]||''),amount=BigInt(log?.args?.[2]||0);if(lower(recipient)!==lower(lane.holder)||amount===0n)continue;
      const tx=await provider.getTransaction(log.transactionHash),decoded=tx?decodeRewardClaimTokenId({to:tx.to,data:tx.data,rewardContract:lane.rewardContract,rewardToken:lane.rewardToken,voter:cfg.voter}):null;
      const proof={blockNumber:Number(log.blockNumber),transactionHash:String(log.transactionHash||''),logIndex:Number(log.index??0),recipient,rewardToken:lane.rewardToken,amountRaw:amount.toString(),decodedTokenId:decoded,decodePath:tx?lower(tx.to)===lower(lane.rewardContract)?'direct-reward-getReward':lower(tx.to)===lower(cfg.voter)?'voter-claimFees-or-claimBribes':'unresolved-top-level-call':'transaction-unavailable'};
      if(decoded!==String(lane.tokenId)){unresolved.push(proof);continue;}total+=amount;events.push(proof);
    }
  }
...
async function rebaseSettlements({contract,lane,fromBlock,toBlock}){
  ...
  for(let from=fromBlock;from<=toBlock;from+=MAX_LOG_BLOCKS){
    ...
    for(const log of logs){const amount=BigInt(log?.args?.[3]||0);total+=amount;events.push({blockNumber:Number(log.blockNumber),transactionHash:String(log.transactionHash||''),logIndex:Number(log.index??0),tokenId:String(log.args?.[0]||lane.tokenId),epochStart:String(log.args?.[1]||''),epochEnd:String(log.args?.[2]||''),amountRaw:amount.toString()});}
  }
...
async function buildProtocolLanes({rewards,cfg,protocolKey,provider,latestNumber,observedAt,prices}){
  const descriptors=trackedPositionDescriptors(rewards).filter(x=>x.protocolKey===protocolKey),lanes=[];
  for(const p of descriptors){
    if(p.mode==='direct'){
      lanes.push({protocolKey,protocol:cfg.protocol,chain:cfg.chain,chainId:cfg.chainId,route:p.route,company:p.company,holder:p.holder,walletAlias:p.walletAlias,tokenId:p.tokenId,custodyContext:p.custodyContext,kind:'rebase-distributor',distributor:cfg.rewardsDistributor,rewardContract:null,rewardToken:getAddress(cfg.baseToken),rewardSymbol:cfg.baseSymbol,decimals:18});
      for(const rewardAddress of p.rewardContracts){
        const c=new Contract(rewardAddress,REWARD_ABI,provider);let tokens=[];try{tokens=await rewardTokens(c,latestNumber);}catch{continue;}
        for(const token of tokens){const meta=await tokenMeta(provider,token,latestNumber);lanes.push({protocolKey,protocol:cfg.protocol,chain:cfg.chain,chainId:cfg.chainId,route:p.route,company:p.company,holder:p.holder,walletAlias:p.walletAlias,tokenId:p.tokenId,custodyContext:p.custodyContext,kind:'voting-reward',rewardContract:getAddress(rewardAddress),distributor:null,rewardToken:getAddress(token),rewardSymbol:meta.symbol,decimals:meta.decimals});}
      }
    }
    if(p.mode==='managed'&&p.freeManagedReward){
      const c=new Contract(p.freeManagedReward,REWARD_ABI,provider);let tokens=[];try{tokens=await rewardTokens(c,latestNumber);}catch{continue;}
      for(const token of tokens){const meta=await tokenMeta(provider,token,latestNumber);lanes.push({protocolKey,protocol:cfg.protocol,chain:cfg.chain,chainId:cfg.chainId,route:p.route,company:p.company,holder:p.holder,walletAlias:p.walletAlias,tokenId:p.tokenId,custodyContext:p.custodyContext,managedTokenId:p.managedTokenId,kind:'free-managed-reward',rewardContract:getAddress(p.freeManagedReward),distributor:null,rewardToken:getAddress(token),rewardSymbol:meta.symbol,decimals:meta.decimals});}
    }
  }
  const seen=new Set();
  return lanes.filter(x=>{x.laneKey=laneKey(x);x.price=prices.get(lower(x.rewardToken))||null;if(seen.has(x.laneKey))return false;seen.add(x.laneKey);return true;});
}
...
async function readLaneState({provider,cfg,lane,blockNumber,observedAt,monthBoundary=false}){
  try{
    ...
    return{ok:true,checkpointKey:checkpointKey(lane,blockNumber),laneKey:lane.laneKey,company:lane.company,protocolKey:lane.protocolKey,protocol:lane.protocol,chain:lane.chain,chainId:lane.chainId,route:lane.route,holder:lane.holder,tokenId:lane.tokenId,custodyContext:lane.custodyContext,kind:lane.kind,rewardContract:lane.rewardContract,distributor:lane.distributor,rewardToken:lane.rewardToken,rewardSymbol:lane.rewardSymbol,decimals:lane.decimals,observedAt,blockNumber,entitlementRaw:BigInt(raw).toString(),entitlementAmount:round(Number(formatUnits(raw,lane.decimals)),12),monthBoundary,exactBlockTaggedState:true,periodIncomeAuthority:false,unknownIsNotZero:true};
...
export async function buildVe33Evidence({rewards,previous={},generatedAt=new Date().toISOString(),providers={}}={}){
  ...
  for(const[protocolKey,cfg]of Object.entries(PROTOCOLS)){
    ...
    const observedAt=new Date(Number(latestBlock.timestamp)*1000).toISOString(),blockCache=new Map(),lanes=await buildProtocolLanes({rewards,cfg,protocolKey,provider,latestNumber,observedAt,prices});diagnostics.laneCount+=lanes.length;
    const pd={latestBlockNumber:latestNumber,observedAt,laneCount:lanes.length,boundaryFailures:[],intervalCount:0,settlementEventCount:0};diagnostics.protocols[protocolKey]=pd;
    const boundaries=monthBoundaries(FULL_ACCOUNTING_START,observedAt);
    for(const lane of lanes){
      for(const boundaryAt of boundaries){
        const b=await blockAtOrBefore(provider,boundaryAt,latestNumber,blockCache),key=checkpointKey(lane,b.blockNumber);if(existing.has(key))continue;
        const state=await readLaneState({provider,cfg,lane,blockNumber:b.blockNumber,observedAt:boundaryAt,monthBoundary:true});if(state.ok)existing.set(key,state);else pd.boundaryFailures.push({laneKey:lane.laneKey,boundaryAt,...state});
      }
    ...
    for(const lane of lanes){
      ...
      for(let i=1;i<rows.length;i++){
        ...
        if(settlements.complete!==true){diagnostics.reconciliationCount++;diagnostics.unresolvedSettlementCount+=(settlements.unresolved||[]).length;continue;}
        ...
        const event={eventKey:key,company:lane.company,family:'accrued-entitlement',economicDate:eventEconomicDate(close),periodStart:open.observedAt,periodEnd:close.observedAt,route:lane.route,protocol:lane.protocol,chain:lane.chain,chainId:lane.chainId,asset:lane.rewardSymbol,token:lane.rewardToken,amount:round(amount,12),amountRaw:r.earnedRaw,usdValue:finite(usdValue)?round(usdValue,8):null,valuationUnitUsd:finite(unitUsd)?round(unitUsd,12):null,valuationAt:lane.price?.observedAt||close.observedAt,valuationStatus:finite(usdValue)?'frozen-at-closing-accounting-boundary':'unvalued-fail-closed',sourceFile:'reporting/ve33-accounting-evidence.json',sourceFamily:'ve(3,3) cumulative entitlement with claim settlement reconciliation',sourceIdentity:`${open.checkpointKey}->${close.checkpointKey}`,evidenceStatus:'factual-opening-plus-settlement-to-closing-reconciliation',mechanismKind:lane.kind,holder:lane.holder,custodyContext:lane.custodyContext,tokenId:lane.tokenId,rewardContract:lane.rewardContract,distributor:lane.distributor,openingEntitlementRaw:open.entitlementRaw,closingEntitlementRaw:close.entitlementRaw,settlementRaw:settlements.amountRaw,settlementEventCount:(settlements.events||[]).length,settlementProofs:settlements.events||[],periodAttributionMonth:month,referenceAprUsed:false,currentClaimableBalanceIsPeriodIncome:false,claimIsSecondIncomeEvent:false,laterClaimOrPriceMoveDoesNotRewriteIncome:true,unknownIsNotZero:true,executionAuthority:'none'};
        priorEvents.set(key,event);diagnostics.acceptedPositiveIntervalCount++;
      }
    }
  }
  ...
  return{version:VERSION,generatedAt,status:partial?'partial':'factual-boundary-tracking',fullAccountingStart:FULL_ACCOUNTING_START,semantics:{openingBalanceCreatesIncome:false,earnedIndependentOfClaim:true,claimIsSettlementNotSecondIncome:true,rebaseDepositIntoVeNftIsSecondIncome:false,referenceAprUsed:false,laterPriceMovementRewritesClosedIncome:false,unknownIsNotZero:true,directVotingRewardFormula:'closing earned + proven ClaimRewards - opening earned',rebaseFormula:'closing claimable + proven Claimed - opening claimable'},scope:{included:['direct veNFT voting fee/incentive reward lanes','direct veNFT RewardsDistributor rebase lanes','managed FreeManagedReward lanes'],deferred:['LockedManagedReward / Relay compounded lane pending exact withdrawal settlement identity','40 Acres payout non-overlap reconciliation with existing Defitea settlement evidence']},authority,checkpoints,events,diagnostics};
}

async function main(){const[rewards,previous]=await Promise.all([readJson(DEFAULT_REWARDS),readJson(DEFAULT_OUTPUT,{})]);const output=await buildVe33Evidence({rewards,previous});await writeJson(DEFAULT_OUTPUT,output);console.log('ve(3,3) factual accounting evidence built',{status:output.status,checkpoints:output.checkpoints.length,events:output.events.length,lanes:output.diagnostics.laneCount,accepted:output.diagnostics.acceptedPositiveIntervalCount,reconciliations:output.diagnostics.reconciliationCount,executionAuthority:output.authority.executionAuthority});}
