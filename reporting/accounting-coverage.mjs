...
function monthEvidence(events,company,hints,family,month){
  return events.filter(e=>e.company===company&&e.family===family&&monthKey(e.economicDate||e.periodEnd)===month&&routeMatches(e,hints));
}
...
function legacyArchive(name,month,monthlyRow){
  return name==='defitea.eth'&&month<='2026-07'&&monthlyRow?.accountingStatus==='complete-legacy-verified-realised';
}
...
async function build(){
  ...
  const output={
    ...
    semantics:{
      productivityCoverageIsNotAccountingCoverage:true,
      referenceMetricIsNotEarnedIncome:true,
      currentRewardStateIsNotPeriodIncome:true,
      factualEvidenceDoesNotImplyFullMechanismCoverage:true,
      partialEvidenceDoesNotCloseMonth:true,
      unknownIsNotZero:true
    },
    completionPolicy:{
      registryHasMonthClosingAuthority:false,
      futureCompletionRequiresAllActiveMechanismsCovered:true,
      futureCompletionRequiresCanonicalValuation:true,
      futureCompletionRequiresNoUnallocatedCrossMonthIntervals:true,
      futureCompletionRequiresNoUnresolvedStateDiscontinuity:true,
      futureCompletionRequiresNoUnknownMechanism:true,
      defiteaLegacyVerifiedRealisedArchivePreserved:true
    },
    sources:{
