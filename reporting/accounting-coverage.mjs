 ...
 * Explicit settlement links reconcile cross-family physical settlements to the
 * active productive mechanism without changing income family or period authority.
 * Supplementary platform routes are isolated to their underlying locked principal.
 */
...
export const ENGINE_CLASS=Object.freeze({
  ...
  fx_vefxn:{family:'accrued-entitlement',mechanism:'governance-rewards',hints:['fxn','vefxn']},
...
export const SUPPLEMENTARY_ROUTE_PRINCIPAL_HINT=Object.freeze({
...
function supplementaryRouteMatches(row,cls){const required=SUPPLEMENTARY_ROUTE_PRINCIPAL_HINT[lower(row?.route)];return !required||(cls?.hints||[]).some(h=>lower(h)===required);}
function eventMatches(e,cls){return supplementaryRouteMatches(e,cls)&&e?.family===cls.family&&textMatches(eventText(e),cls.hints);}
function stateMatches(r,cls){return supplementaryRouteMatches(r,cls)&&textMatches(stateText(r),cls.hints);}
...
function pushTrackingProof(out,{engineId,company,observedAt,sourceFile,proofKey,observationComplete=true,observationCoverage=null}){const canonical=canonicalCompanyName(company),month=monthKey(observedAt);if(!engineId||!canonical||!month)return;out.push({engineId,company:canonical,month,observedAt,sourceFile,proofKey:proofKey||null,observationComplete:observationComplete!==false,observationCoverage:observationCoverage||null});}
...
export function icpNnsObservationProofs(state={},config={}){
  ...
  const requested=Number(observation?.requestedNeuronCount),detailOkCount=Number(observation?.detailOkCount);
  if(requested!==ids.length||neurons.length!==ids.length||!Number.isSafeInteger(detailOkCount)||detailOkCount<=0||detailOkCount>ids.length)return out;
  const observedIds=neurons.map(x=>String(x?.neuronId||''));
  if(observedIds.some(x=>!x)||new Set(observedIds).size!==ids.length||JSON.stringify([...observedIds].sort())!==JSON.stringify([...ids].sort()))return out;
  const actualOk=neurons.filter(x=>x?.detailStatus==='ok').length;
  if(actualOk!==detailOkCount)return out;
  const unavailable=neurons.filter(x=>x?.detailStatus!=='ok');
  if(unavailable.some(x=>x?.detailStatus!=='unavailable'||x?.state!=='Unknown'||x?.stakeIcp!==null||!Array.isArray(x?.errors)||x.errors.length===0))return out;
  const fullDetailCoverage=detailOkCount===ids.length;
  if(observation?.fullDetailCoverage!==fullDetailCoverage)return out;
  if(!fullDetailCoverage&&state?.status!=='partial')return out;
  ...
  const coverage={requestedNeuronCount:ids.length,detailOkCount,unavailableDetailCount:ids.length-detailOkCount,fullDetailCoverage};
  for(const[company]of allocations)pushTrackingProof(out,{engineId:'icp_nns',company,observedAt,sourceFile:'companies/icp-nns-rewards-state.json',proofKey:`icp-nns:public-observation:${detailOkCount}/${ids.length}:${observedAt}`,observationComplete:fullDetailCoverage,observationCoverage:coverage});
  return out;
}
...
export function gmxV2MarketObservationProofs(state={}){
  const out=[];
  const company='Cypher',expectedIds=['gmx-gm-btc-usdc','gmx-gm-eth-usdc'];
  const approx=(a,b,tolerance=1e-9)=>finite(a)&&finite(b)&&Math.abs(Number(a)-Number(b))<=tolerance;
  const addressOk=v=>/^0x[0-9a-f]{40}$/.test(lower(v));
  const gmx=state?.strategies?.gmx,provenance=state?.provenance?.gmxStrategy||{},epistemic=state?.epistemicBoundary||{};
  if(state?.company?.registry!=='010'||state?.company?.name!==company||gmx?.version!=='0.1.1-company-010-gmx-strategy-intelligence-pool-key-fix'||gmx?.status!=='measured')return out;
  if(state?.authority?.executionAuthority!=='none'||state?.authority?.transactions!==false||state?.authority?.capitalMovement!==false||gmx?.authority?.readOnly!==true||gmx?.authority?.noTransactions!==true||gmx?.authority?.executionAuthority!=='none')return out;
  const boundary=gmx?.accountingBoundary||{},rewardsBoundary=gmx?.rewardsBoundary||{};
  if(boundary?.underlyingPoolTokensAreExposureOnly!==true||boundary?.underlyingPoolTokensAddedToReserveBalances!==false||boundary?.gmTokenNavCountedOnce!==true||boundary?.noDoubleCount!==true)return out;
  if(rewardsBoundary?.lpFeeIncome!=='embedded-in-gm-nav'||rewardsBoundary?.separateClaimableLpFees!==false||rewardsBoundary?.claimableAmount!==null||rewardsBoundary?.claimableStatus!=='not-applicable-by-mechanism')return out;
  if(epistemic?.unknownIsNotZero!==true||epistemic?.referenceAprIsNotRealisedIncome!==true||epistemic?.noDoubleCount!==true||epistemic?.noTransactions!==true||epistemic?.gmxUnderlyingExposureIsNotAdditiveCapital!==true||epistemic?.gmxEmbeddedIncomeIsNotClaimableReward!==true)return out;
  if(provenance?.version!==gmx.version||!monthKey(provenance?.generatedAt)||!addressOk(provenance?.dataStore)||!lower(provenance?.poolAmountKeyMethod).includes('pool_amount'))return out;
  const strategies=Array.isArray(gmx?.strategies)?gmx.strategies:[],ids=strategies.map(x=>x?.id).sort();
  if(strategies.length!==2||Number(gmx?.strategyCount)!==2||JSON.stringify(ids)!==JSON.stringify(expectedIds))return out;
  const embedded=(state?.rewards?.embeddedIncomeMechanisms||[]).filter(x=>x?.id==='gmx-v2-market-liquidity');
  if(embedded.length!==1)return out;
  const embeddedRow=embedded[0],embeddedIds=Array.isArray(embeddedRow?.positionIds)?[...embeddedRow.positionIds].sort():[];
  if(embeddedRow?.protocol!=='GMX'||embeddedRow?.chain!=='Arbitrum'||embeddedRow?.incomeMode!=='embedded-in-gm-nav'||embeddedRow?.claimableApplicable!==false||embeddedRow?.claimableAmount!==null||embeddedRow?.status!=='measured-mechanism'||JSON.stringify(embeddedIds)!==JSON.stringify(expectedIds))return out;
  const marketTokens=new Set();
  for(const strategy of strategies){
    const engineId=strategy?.id;
    if(!expectedIds.includes(engineId)||strategy?.protocol!=='GMX'||strategy?.chain!=='Arbitrum'||!addressOk(strategy?.marketToken)||marketTokens.has(lower(strategy.marketToken)))continue;
    marketTokens.add(lower(strategy.marketToken));
    const position=strategy?.companyPosition||{},yieldState=strategy?.yield||{},exposure=strategy?.poolExposureDiagnostic||{};
    if(!(Number(position?.gmBalance)>0)||!(Number(position?.gmTokenPriceUsd)>0)||!(Number(position?.strategyNavUsd)>0)||!lower(position?.capitalAccounting).includes('count gm token strategy nav once'))continue;
    if(yieldState?.referenceMetric!=='GMX 30D Fee APY'||yieldState?.incomeMode!=='embedded-in-gm-nav'||yieldState?.claimableApplicable!==false||yieldState?.separateClaimStep!==false||yieldState?.annualizedPerformanceRole!=='diagnostic-only-not-reference-yield')continue;
    if(!lower(yieldState?.economics).includes('increase gm token value')||!lower(yieldState?.economics).includes('no separate lp fee claim'))continue;
    const long=exposure?.long||{},short=exposure?.short||{};
    if(!lower(exposure?.source).includes('gmx datastore pool_amount')||!lower(exposure?.interpretation).includes('not additive company capital'))continue;
    if(!addressOk(long?.address)||!addressOk(short?.address)||!(Number(long?.amount)>0)||!(Number(short?.amount)>0)||!(Number(long?.usd)>0)||!(Number(short?.usd)>0)||!finite(long?.grossTokenWeight)||!finite(short?.grossTokenWeight)||!approx(Number(long.grossTokenWeight)+Number(short.grossTokenWeight),1,0.00001)||!(Number(exposure?.grossPoolTokenValueUsd)>0))continue;
    const capital=(state?.capital?.positions||[]).find(x=>x?.assetId===engineId);
    if(!capital||capital?.protocol!=='GMX'||capital?.chain!=='Arbitrum'||capital?.strategyClass!=='GMX V2 market liquidity'||lower(capital?.marketToken)!==lower(strategy.marketToken)||!lower(capital?.source).includes('official gmx reader')||!lower(capital?.capitalRule).includes('count gm token strategy nav once'))continue;
    if(!approx(capital?.quantity,position.gmBalance,1e-9)||!approx(capital?.valueUsd,position.strategyNavUsd,0.01))continue;
    const productivity=(state?.productivity?.positions||[]).find(x=>x?.id===engineId);
    if(!productivity||productivity?.incomeMode!=='embedded-in-gm-nav'||productivity?.claimableApplicable!==false||productivity?.referenceMetric!=='GMX 30D Fee APY'||!approx(productivity?.quantity,position.gmBalance,1e-9)||!approx(productivity?.valueUsd,position.strategyNavUsd,0.01))continue;
    const observedAt=provenance.generatedAt||gmx?.generatedAt||state?.generatedAt;
    if(!monthKey(observedAt))continue;
    pushTrackingProof(out,{engineId,company,observedAt,sourceFile:'companies/company-010-production-state.json#strategies.gmx',proofKey:`gmx-v2-market:${lower(strategy.marketToken)}:${Number(position.gmBalance).toFixed(12)}:${observedAt}`});
  }
  return out;
}
...
export function factualTrackingProofs({ve33={},ve33LockedManaged={},yieldBasis={},frax={},rewards={},ledger={},icpNnsState={},icpNnsConfig={},company010State={}}={}){
  ...
  out.push(...strongVlCvxRouteProofs(rewards),...strongCvxCrvClaimableProofs(rewards),...pendleSPendleObservationProofs(rewards),...veniceSVvvObservationProofs(rewards),...resupplyRsupObservationProofs(rewards),...liquityLqtyObservationProofs(rewards),...hyperlendKhypeObservationProofs(company010State,rewards),...projectXWhypeUsdcObservationProofs(company010State),...concentratorAsdCrvObservationProofs(company010State),...gmxV2MarketObservationProofs(company010State),...curveFeeAccrualProofs(ledger),...icpNnsObservationProofs(icpNnsState,icpNnsConfig));
...
function mechanismCoverage({events,stateRows,trackingProofs,company,engineId,cls,month}){
  ...
  const partialProofs=proofs.filter(p=>p?.observationComplete===false);
  ...
  const crossMonthExplicitlyAttributed=crossMonth.filter(e=>explicitMonth.test(String(e?.periodAttributionMonth||''))&&e.periodAttributionMonth===month);
  const attributedKeys=new Set(crossMonthExplicitlyAttributed.map(e=>e?.eventKey||`${e?.periodStart}|${e?.periodEnd}|${e?.sourceIdentity||''}`));
  const crossMonthUnresolved=crossMonth.filter(e=>!attributedKeys.has(e?.eventKey||`${e?.periodStart}|${e?.periodEnd}|${e?.sourceIdentity||''}`));
  const effectiveValuations=factual.map(e=>({event:e,usdValue:canonicalResolvedUsdValue(e)}));
  const valued=effectiveValuations.filter(x=>finite(x.usdValue)),factualTrackingActive=factual.length>0||proofs.length>0;
  ...
  return{status,factualTrackingActive,factualTrackingProofCount:proofs.length,factualTrackingProofSources:unique(proofs.map(p=>p.sourceFile)).sort(),factualTrackingPartialObservationCount:partialProofs.length,factualTrackingObservationComplete:proofs.length?partialProofs.length===0:null,factualTrackingObservationCoverage:partialProofs.length?partialProofs.map(p=>p.observationCoverage).filter(Boolean):[],factualEventCount:factual.length,factualValuedEventCount:valued.length,factualUsdSubtotal:factual.length&&valued.length===factual.length?Number(valued.reduce((s,x)=>s+Number(x.usdValue),0).toFixed(8)):null,currentStateRouteCount:state.length,crossMonthEvidenceCount:crossMonth.length,crossMonthExplicitlyAttributedCount:crossMonthExplicitlyAttributed.length,crossMonthUnresolvedCount:crossMonthUnresolved.length,firstFactualEvidenceAt:factual.length?factual.map(e=>e?.periodStart||e?.economicDate||e?.periodEnd).filter(Boolean).sort()[0]||null:null,lastFactualEvidenceAt:factual.length?factual.map(e=>e?.periodEnd||e?.economicDate).filter(Boolean).sort().at(-1)||null:null,mechanismCompleteForMonth:false,completionBlockers:unique([...(factual.length?[]:['no-canonical-period-income-evidence']),...(factualTrackingActive?[]:['no-factual-engine-tracking-proof']),...(state.length&&!factualTrackingActive?['current-state-is-not-period-income']:[]),...(partialProofs.length?['current-source-observation-partial']:[]),...(crossMonthUnresolved.length?['cross-month-boundary-requires-explicit-allocation']:[]),...(cls.classified?[]:['unclassified-income-mechanism'])])};
}
...
function mechanismAggregate(engineId,rows,currentMonth){
  ...
  return{engineId,accountingFamily:sample.accountingFamily||'unknown',mechanismType:sample.mechanismType||'unclassified',protocols:unique(rows.map(x=>x.mechanism?.protocol)).sort(),activeCompanyCount:companies.length,companies,knownProductiveValueUsdTotal:sumKnown(rows.map(x=>x.mechanism?.productiveValueUsd)),knownProductiveValueCompanyCount:rows.filter(x=>finite(x.mechanism?.productiveValueUsd)).length,factualTrackingCompanyCount:trackingCompanies.length,factualTrackingCompanies:trackingCompanies,factualEventCompanyCount:eventCompanies.length,factualEventCompanies:eventCompanies,factualCompanyCount:trackingCompanies.length,factualCompanies:trackingCompanies,stateOnlyCompanyCount:stateOnlyCompanies.length,stateOnlyCompanies,referenceOnlyCompanyCount:referenceOnlyCompanies.length,referenceOnlyCompanies,currentMonthFactualEventCount:current.reduce((s,x)=>s+Number(x?.factualEventCount||0),0),currentMonthFactualUsdSubtotal:current.length&&current.every(x=>x.factualEventCount===0||finite(x.factualUsdSubtotal))?Number(current.reduce((s,x)=>s+Number(x?.factualUsdSubtotal||0),0).toFixed(8)):null,currentMonthPartialObservationCompanyCount:current.filter(x=>Number(x?.factualTrackingPartialObservationCount||0)>0).length,reusableCoverageGap:trackingCompanies.length<companies.length,referenceMetricIsAccountingAuthority:false,completionAuthority:false};
}

export function explicitSettlementMechanismLinks(events=[],mechanismRows=[]){
  const out=[];
  for(const e of events||[]){
    const company=canonicalCompanyName(e?.company);
    if(company!=='defitea.eth'||e?.family!=='realised-cash-flow'||e?.route!=='forty-acres-velodrome-received'||e?.protocol!=='40 Acres · veVELO')continue;
    if(e?.sourceFile!=='reporting/defitea-income-ledger.json'||e?.sourceFamily!=='fortyAcresReceivedEvents'||e?.evidenceStatus!=='canonical-actual-net-received')continue;
    if(e?.executionAuthority!=='none'||e?.retention!=='indefinite'||e?.laterStateChangeDoesNotEraseIncome!==true)continue;
    if(!(finite(e?.amount)&&Number(e.amount)>0)||!(finite(e?.usdValue)&&Number(e.usdValue)>0)||!/^([0-9a-f]{64})$/i.test(String(e?.immutableEconomicFieldsHash||'')))continue;
    const identity=String(e?.sourceIdentity||''),m=identity.match(/^40acres:(0x[0-9a-f]{64}):(\d+)$/i);
    if(!m||e?.eventKey!==`defitea-received:${identity}`)continue;
    const expectedPhysical=`10:${lower(m[1])}:${Number(m[2])}`;
    if(lower(e?.physicalEventId)!==expectedPhysical)continue;
    const active=mechanismRows.find(x=>companyMatches(x?.company,company)&&x?.mechanism?.engineId==='velodrome_vevelo'&&x?.mechanism?.accountingFamily==='accrued-entitlement'&&x?.mechanism?.mechanismType==='governance-rewards'&&x?.mechanism?.classified===true);
    if(!active)continue;
    out.push({eventKey:e.eventKey,company,engineId:'velodrome_vevelo',relation:'realised-settlement-to-active-productive-mechanism',sourceAuthority:'canonical-income-ledger',sourceFile:e.sourceFile,sourceFamily:e.sourceFamily,sourceIdentity:e.sourceIdentity,physicalEventId:e.physicalEventId,incomeFamily:'realised-cash-flow',incomeFamilyPreserved:true,periodIncomeAuthority:false,factualTrackingAuthority:false,referenceMetricAuthority:false,executionAuthority:'none'});
  }
  return out.sort((a,b)=>a.eventKey.localeCompare(b.eventKey));
}

function unmatchedEvents(events,mechanismRows,settlementLinks=[]){
  const known=mechanismRows.map(x=>({company:x.company,cls:{family:x.mechanism.accountingFamily,hints:x.mechanism.accountingRouteHints}})),linked=new Set((settlementLinks||[]).map(x=>x.eventKey));
  return events.filter(e=>!known.some(k=>companyMatches(e?.company,k.company)&&eventMatches(e,k.cls))&&!linked.has(e?.eventKey));
}
...
export function buildAccountingCoverage({productivity={},ledger={},embedded={},factualEvidence={},generatedAt=null}={}){
  ...
  const settlementLinks=explicitSettlementMechanismLinks(events,flat),unmatched=unmatchedEvents(events,flat,settlementLinks),unclassified=flat.filter(x=>x.mechanism.classified!==true),gaps=Object.values(mechanisms).filter(x=>x.reusableCoverageGap).sort((a,b)=>b.activeCompanyCount-a.activeCompanyCount||(Number(b.knownProductiveValueUsdTotal||0)-Number(a.knownProductiveValueUsdTotal||0))||a.engineId.localeCompare(b.engineId)).map((x,index)=>({rank:index+1,engineId:x.engineId,activeCompanyCount:x.activeCompanyCount,knownProductiveValueUsdTotal:x.knownProductiveValueUsdTotal,factualTrackingCompanyCount:x.factualTrackingCompanyCount,factualEventCompanyCount:x.factualEventCompanyCount,factualCompanyCount:x.factualCompanyCount,stateOnlyCompanyCount:x.stateOnlyCompanyCount,referenceOnlyCompanyCount:x.referenceOnlyCompanyCount}));
  ...
  return{version:VERSION,generatedAt:at,status:'diagnostic-no-completion-authority',currentMonth,sourceFreshness:{productivityGeneratedAt:productivity?.generatedAt||null,incomeLedgerGeneratedAt:ledger?.generatedAt||null,embeddedYieldGeneratedAt:embedded?.generatedAt||null,rewardsGeneratedAt:factualEvidence?.rewards?.generatedAt||null,ve33EvidenceGeneratedAt:factualEvidence?.ve33?.generatedAt||null,ve33LockedManagedEvidenceGeneratedAt:factualEvidence?.ve33LockedManaged?.generatedAt||null,yieldBasisEvidenceGeneratedAt:factualEvidence?.yieldBasis?.generatedAt||null,fraxEvidenceGeneratedAt:factualEvidence?.frax?.generatedAt||null,icpNnsStateGeneratedAt:factualEvidence?.icpNnsState?.generatedAt||null,company010ProjectXGeneratedAt:factualEvidence?.company010State?.strategies?.projectX?.generatedAt||null,company010GmxGeneratedAt:factualEvidence?.company010State?.strategies?.gmx?.generatedAt||null,company010StateGeneratedAt:factualEvidence?.company010State?.provenance?.hyperlendIncome?.generatedAt||factualEvidence?.company010State?.generatedAt||null},semantics:{canonicalLedgerIsSoleFactualIncomeAuthority:true,canonicalHistoricalValuationResolutionReusedForDiagnosticUsd:true,productivityCoverageIsNotAccountingCoverage:true,referenceMetricIsNotEarnedIncome:true,currentRewardStateIsNotPeriodIncome:true,factualTrackingProofIsNotPeriodIncome:true,trackingCapabilityIsDistinctFromCurrentObservationCompleteness:true,partialCurrentObservationCanProveCapabilityWithoutProvingCompleteState:true,partialCurrentObservationCannotCreateIncome:true,routeStateCanProveTrackingOnlyWithSettlementProof:true,unresolvedSettlementCannotProveFactualTracking:true,settlementLinkDoesNotChangeIncomeFamily:true,settlementLinkIsNotPeriodIncomeAuthority:true,supplementaryRoutePrincipalIsolation:true,genericSupplementaryRouteHintCannotCrossPrincipal:true,supplementaryPlatformLabelDoesNotCreatePrincipalIdentity:true,zeroPeriodEventDoesNotImplyCoverageGap:true,coverageGapMeansMissingFactualTrackingCapability:true,factualEvidenceDoesNotImplyFullMechanismCoverage:true,partialEvidenceDoesNotCloseMonth:true,explicitCanonicalPeriodAttributionResolvesDiagnosticCrossMonthBoundary:true,crossMonthWithoutExplicitCanonicalAttributionRemainsUnresolved:true,unknownIsNotZero:true,newCompanyDoesNotRequireNewAccountingEngineWhenMechanismAlreadySupported:true,unclassifiedMechanismIsVisibleGapNotZero:true,historicalCompanyAliasesCanonicalized:true,stringAliasCannotCreateNewCompanyIdentity:true},completionPolicy:{registryHasMonthClosingAuthority:false,registryHasIncomeCreationAuthority:false,coverageGapRankingIsDiagnosticOnly:true,settlementLinksAreDiagnosticOnly:true},authority:{executionAuthority:'none',walletAuthority:'none',claimingAuthority:'none',capitalExecution:false,monthClosingAuthority:false,methodologyMutationAuthority:'none'},summary:{companyCount:companyNames.length,mechanismInstanceCount:flat.length,uniqueMechanismCount:mechanismIds.length,classifiedMechanismInstanceCount:flat.length-unclassified.length,unclassifiedMechanismInstanceCount:unclassified.length,reusableCoverageGapCount:gaps.length,currentMonthPartialObservationMechanismCount:Object.values(companies).reduce((sum,c)=>sum+Number(c.currentMonthPartialObservationMechanismCount||0),0),canonicalLedgerEventCount:events.length,settlementLinkedCanonicalEventCount:settlementLinks.length,unmatchedCanonicalEventCount:unmatched.length,canonicalizedAliasCompanyCount:aliasObservations.length,factualTrackingProofCount:tracking.length},companyIdentityAliases:aliasObservations,gapRanking:gaps,settlementMechanismLinks:settlementLinks,unmatchedCanonicalEvents:unmatched.slice(0,50).map(e=>({eventKey:e?.eventKey||null,company:canonicalCompanyName(e?.company)||null,sourceCompany:e?.company||null,family:e?.family||null,protocol:e?.protocol||null,route:e?.route||null,sourceFile:e?.sourceFile||null})),mechanisms,companies};
}
