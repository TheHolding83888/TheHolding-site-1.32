...
export const VERSION='0.4-yield-basis-lp-factual-tracking-accounting-mechanism-coverage-registry';
...
export function yieldBasisLpObservationProofs(evidence={}){
  const out=[];
  const expected={yieldbasis_yblp_wbtc:{market:'yb-WBTC',lt:'0x651d4b8168488fa163d85304662e8278d4c55baa'},yieldbasis_yblp_weth:{market:'yb-WETH',lt:'0x2b9c9f3bdceb5d8e36a4704f08a78fca53343cea'}};
  if(evidence?.version!=='0.1-yield-basis-lp-factual-tracking'||!['ok','partial'].includes(evidence?.status))return out;
  if(evidence?.semantics?.factualTrackingProofIsNotPeriodIncome!==true||evidence?.semantics?.openingBalanceCreatesIncome!==false||evidence?.semantics?.referenceAprUsed!==false||evidence?.semantics?.unknownIsNotZero!==true)return out;
  if(evidence?.authority?.readOnly!==true||evidence?.authority?.executionAuthority!=='none'||evidence?.authority?.walletAuthority!=='none'||evidence?.authority?.claimingAuthority!=='none'||evidence?.authority?.capitalExecution!==false)return out;
  for(const row of Array.isArray(evidence?.checkpoints)?evidence.checkpoints:[]){
    const want=expected[row?.engineId],holders=Array.isArray(row?.holders)?row.holders:[];
    ...
    if(row?.company!=="Rook's portfolio"||String(row?.registry)!=='007'||row?.chain!=='Ethereum'||row?.protocol!=='Yield Basis'||row?.market!==want.market||lower(row?.lt)!==want.lt)continue;
    ...
    if(!holders.length||holders.some(x=>!/^0x[0-9a-f]{40}$/.test(lower(x?.wallet))||!/^0x[0-9a-f]+$/i.test(String(x?.balanceRaw||'')))||!holders.some(x=>BigInt(x.balanceRaw)>0n))continue;
...
export function factualTrackingProofs({ve33={},ve33LockedManaged={},yieldBasis={},yieldBasisLp={},frax={},rewards={},ledger={},icpNnsState={},icpNnsConfig={}}={}){
  const out=[];
  ...
  out.push(...strongVlCvxRouteProofs(rewards),...strongCvxCrvClaimableProofs(rewards),...pendleSPendleObservationProofs(rewards),...veniceSVvvObservationProofs(rewards),...yieldBasisLpObservationProofs(yieldBasisLp),...curveFeeAccrualProofs(ledger),...icpNnsObservationProofs(icpNnsState,icpNnsConfig));
...
async function main(){
  const[productivity,ledger,embedded,rewards,ve33,ve33LockedManaged,yieldBasis,yieldBasisLp,frax,icpNnsState,icpNnsConfig]=await Promise.all([readJson(PRODUCTIVITY_FILE),readJson(INCOME_LEDGER_FILE),readJson(EMBEDDED_FILE),readOptionalJson(REWARDS_FILE),readOptionalJson(VE33_EVIDENCE_FILE),readOptionalJson(VE33_LOCKED_MANAGED_EVIDENCE_FILE),readOptionalJson(YIELD_BASIS_EVIDENCE_FILE),readOptionalJson(YIELD_BASIS_LP_EVIDENCE_FILE),readOptionalJson(FRAX_EVIDENCE_FILE),readOptionalJson(ICP_NNS_STATE_FILE),readOptionalJson(ICP_NNS_CONFIG_FILE)]);
  const output=buildAccountingCoverage({productivity,ledger,embedded,factualEvidence:{rewards,ve33,ve33LockedManaged,yieldBasis,yieldBasisLp,frax,icpNnsState,icpNnsConfig}});
