#!/usr/bin/env node
/**
 * The Holding · Accounting Coverage Registry v0.11
 *
 ...
 * Current positive reward state is exposed only as a diagnostic observation;
 * it never proves that the visible amount belongs to the selected period.
...
export const VERSION='0.11-current-reward-period-status-accounting-mechanism-coverage-registry';
...
function mechanismCoverage({events,stateRows,trackingProofs,company,engineId,cls,month}){
  ...
  const positiveState=state.filter(r=>(finite(r?.amount)&&Number(r.amount)>0)||(finite(r?.usdValue)&&Number(r.usdValue)>0));
  const valuedPositiveState=positiveState.filter(r=>finite(r?.usdValue));
  const positiveStateUsdSubtotal=positiveState.length&&valuedPositiveState.length===positiveState.length?Number(valuedPositiveState.reduce((sum,r)=>sum+Number(r.usdValue),0).toFixed(8)):null;
