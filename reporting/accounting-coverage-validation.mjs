...
assert.equal(isolated.semantics?.canonicalHistoricalValuationResolutionReusedForDiagnosticUsd,true);
assert.ok(curve&&fx);
...
// The 19-event / $2.04403678 snapshot is a proven historical floor, not a permanent
// ceiling: later factual rebuilds may legitimately discover additional unique evidence.
// Coverage must preserve the proven floor and value every admitted factual event rather
