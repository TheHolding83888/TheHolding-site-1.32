...
assert.deepEqual(reconcileEntitlement('100','150','0'),{status:'positive-factual-accrual',accepted:true,earnedRaw:'50'});
assert.deepEqual(reconcileEntitlement('100','10','100'),{status:'positive-factual-accrual',accepted:true,earnedRaw:'10'});
assert.deepEqual(reconcileEntitlement('100','0','100'),{status:'zero-new-earned',accepted:true,earnedRaw:'0'});
assert.deepEqual(reconcileEntitlement('100','20','80'),{status:'zero-new-earned',accepted:true,earnedRaw:'0'});
...
const rewards={generatedAt:'2026-09-03T06:00:00.000Z',companies:{
  'Example.eth':{
    ...
    rewards:[{route:'yield-basis-fees',token:'0x0000000000000000000000000000000000000011',symbol:'yb-WBTC',priceUsd:74250.5,priceMethod:'redemption-value:WBTC@canonical',details:{wallet:'0x0000000000000000000000000000000000000002',walletAlias:'Secondary'}}]
  }
}};
...
const requested=[];
const fakeProvider={async getBlock(number){requested.push(Number(number));return{number:Number(number),timestamp:Number(number)*12};}};
...
console.log('Yield Basis factual accrual evidence validation OK',{
  version:VERSION,feeDistributor:DISTRIBUTOR,
  formula:'closing preview_claim + Claim settlements - opening preview_claim, token by token',
  openingBalanceCreatesIncome:false,claimIsSettlementNotSecondIncome:true,claimToZeroHandled:true,
  zeroWalletStatePreserved:true,recentBoundarySearch:true,boundaryLookupReads:requested.length,
  oldestBoundaryProbe:Math.min(...requested),referenceAprUsed:false,unknownIsNotZero:true
});

// TEMPORARY diagnostic: never fails validation. Remove once historical getter
// failure is localized and the production storage-derived fallback is corrected.
try{
  ...
  if(initial!==null&&last!==null){
    ...
    if(veAddress){
      const ve=new Contract(veAddress,['function getPastVotes(address,uint256) view returns (uint256)','function getPastTotalSupply(uint256) view returns (uint256)'],rpc);
...
}catch(error){console.log('YB_STORAGE_DIAG OUTER_FAIL',error?.shortMessage||error?.message||String(error));}
