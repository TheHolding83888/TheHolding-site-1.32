...
(() => {
  ...
  function protocols(){
    const p=['Bitcoin','Ethereum','Hyperliquid','Project X','Convex','Curve','Aerodrome','Velodrome','GMX','Concentrator','Fluid'];
    if((state?.capital?.positions||[]).some(x=>x.protocol==='HyperLend'||String(x.assetId||'').startsWith('hyperlend-')))p.splice(4,0,'HyperLend');
    return p;
  }
  ...
  function cardCopy(){return lang()==='ru'
    ?'Компания Bitcoin Standard с резервным капиталом и продуктивными позициями в Aave, HyperLend, Convex, Aerodrome, Velodrome, GMX, Project X и Concentrator.'
    :'A Bitcoin Standard company combining reserve capital with productive positions across Aave, HyperLend, Convex, Aerodrome, Velodrome, GMX, Project X and Concentrator.';}
  ...
  const run=()=>start().catch(err=>console.error('[Company #010 reconciled native public adapter]',err));
