...
STRICT SHAPE RULES:
- entities MUST be an array of plain strings only, for example ["Monetra","Defitea"]. Never emit entity objects, type wrappers, ids, maps or nested structures.
- every top-level field is scalar except entities, decomposition and missingPrimitives.
- requestedMetric MUST be one allowed string, never an array. If multiple distinct metrics/concepts are requested, set requestedMetric="none" and preserve each semantic part in decomposition.
- each decomposition item may contain only object, entity, operation, concept. entity there is also a plain string or omitted. concept is allowed only when object="unmodeled".
...
- preserve entity role across the entire conversation. A protocol mentioned earlier remains a protocol; protocol→companies→ranking requires protocol-state plus the requested metric/ranking.
- use intent="composite" for 2+ supported semantic subparts.
- use intent="unsupported-decomposed" when at least one required primitive is unavailable; include each unavailable primitive both in decomposition and missingPrimitives.
- use intent="authority-boundary" whenever any requested subpart asks to move capital, sign, transact, bypass guards, mutate policy or exercise operational authority. Authority dominates mixed fact+execution requests. For authority-boundary, decomposition and missingPrimitives must be empty.
...
const objectSet = envelope => new Set((envelope?.decomposition || []).map(item => item.object));
...
for (const test of selected) {
  ...
  const fit = {
    ...
    authority: !!envelope && (!test.authorityDominates || (envelope.intent === 'authority-boundary' && envelope.decomposition.length === 0 && envelope.missingPrimitives.length === 0))
  };
  ...
  const found = required.filter(item => objects.has(item)).length;
  const extraObjects = envelope && required.length ? envelope.decomposition.map(x => x.object).filter(item => !required.includes(item)) : [];
...
const summary = {
  version: '0.3-compositional-model-shadow-evaluation',
  ...
  extraDecompositionObjects: results.reduce((sum, r) => sum + r.diagnostics.extraObjects.length, 0),
