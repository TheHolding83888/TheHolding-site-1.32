...
SCALAR CONTRACT RULE:
Every top-level field is scalar except entities, decomposition and missingPrimitives. requestedMetric MUST be exactly one allowed string, never an array. If a compound request asks for multiple distinct metrics/concepts, use requestedMetric="none" and preserve every requested semantic part in decomposition.

COMPARISON RULE:
Use top-level operation="compare" for an ordinary A-vs-B comparison. comparison stays "none" unless the user requests a directional/ranking form such as highest, lowest, largest or most-concentrated. Words like "higher", "which has more", "у какой больше" imply the appropriate directional comparison when available.

MULTI-TURN ROLE RULE:
Preserve entity type and semantic role across the entire conversation. A protocol mentioned in an earlier turn remains a protocol in later references. If the conversation asks about a protocol, which companies use it, and then ranks those companies, decomposition must preserve protocol-state plus the requested company metric/ranking. Do not convert the protocol itself into company-identity merely because later turns discuss companies.
...
function renderQuestion(test) {
  if (Array.isArray(test.session)) return test.session.map((turn, index) => `TURN ${index + 1}: ${turn}`).join('\n');
...
function objectSet(envelope) {
  return new Set((envelope?.decomposition || []).map((item) => item.object));
}

function requiredObjectsFit(envelope, required = []) {
  if (!required.length) return true;
  const objects = objectSet(envelope);
...
for (const test of selectedCases) {
  ...
  const operationFit = !!envelope && (!test.expectedOperation || envelope.operation === test.expectedOperation);
  ...
  const strictFit = !!envelope && intentFit && objectsFit && missingFit && entitiesFit && comparisonFit && operationFit && conceptFit && authorityFit;

  const requiredCount = (test.requiredObjects || []).length;
  const objects = envelope ? objectSet(envelope) : new Set();
  const foundCount = (test.requiredObjects || []).filter((item) => objects.has(item)).length;
  ...
  results.push({
    ...
    fit: { intent: intentFit, requiredObjects: objectsFit, missingPrimitives: missingFit, entities: entitiesFit, comparison: comparisonFit, operation: operationFit, concept: conceptFit, authority: authorityFit, strict: strictFit },
...
const operationEvaluated = results.filter((r) => r.expected.operation).length;
const operationCorrect = results.filter((r) => r.expected.operation && r.fit.operation).length;
...
const summary = {
  version: '0.2-compositional-model-shadow-evaluation',
