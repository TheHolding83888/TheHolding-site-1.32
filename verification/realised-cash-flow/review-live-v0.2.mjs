...
if (x?.version !== '0.2.2-yield-basis-live-realised-cash-flow') fail(`unexpected version ${x?.version}`);
...
if (!String(x?.scan?.providers?.historicalLogs || '').startsWith('logs:')) fail('historical log provider provenance missing');
...
if (!Number.isInteger(Number(x?.scan?.windowCount)) || Number(x.scan.windowCount) <= 0) fail('invalid historical window count');
...
console.log(JSON.stringify({
  status: 'PASS', version: '0.2.2-independent-live-realised-cash-flow-review', deploymentReceiptVerified: true,
