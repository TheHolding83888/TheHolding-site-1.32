...
aero.sourceState={
  ...(aero.sourceState||{}),
  canonicalCompanyState:'companies/yieldring-canonical-state.json',
  principalQuantity:target,
  relayMode:state.aerodromeRelay?.mode||null,
  expectedUnderlyingLockCount:state.aerodromeRelay?.expectedUnderlyingLockCount??null,
  evidenceStatus:state.aerodromeRelay?.evidenceStatus||null
};
...
if(Array.isArray(history)&&history.length){
  ...
  if(latest&&latest.snapshotKey===data.snapshotKey){
    latest.apr=company.aprLatest;
    if('totalProductiveValue' in latest)latest.totalProductiveValue=company.productiveValue;
    if('coveredProductiveValue' in latest)latest.coveredProductiveValue=company.coveredProductiveValue;
    if('coverage' in latest)latest.coverage=company.coverage;
  }
...
data.diagnostics.yieldRing={
  version:'0.1-canonical-capital-and-relay-overlay',
  source:'companies/yieldring-canonical-state.json',
  bitcoinQuantity:Number(state.capital.bitcoin.quantity),
  bitcoinCostBasisUsd:Number(state.capital.bitcoin.costBasisUsd),
  aeroQuantity:target,
  aeroCostBasisUsd:Number(state.capital.aerodrome.costBasisUsd),
  ...
  rewardsPresentation:state.aerodromeRelay.rewardsPresentation,
