...
async function main(){
  ...
  for(const pos of raw){
    ...
    const p0=n(pos.principalAmount0),p1=n(pos.principalAmount1);if(p0===null||p1===null)throw new Error(`Project X NFT ${pos.tokenId} principal leg missing`);
    const pWhype=w0?p0:p1,pUsdc=u0?p0:p1,navUsd=pWhype*hypePx+pUsdc*usdcPx;
    if(!(navUsd>=MIN_ACTIVE_NAV_USD)){ignored.push({tokenId:String(pos.tokenId),reason:'dust-or-empty',navUsd:round(navUsd,6)});continue}
  ...
  const p=await getProvider(),npm=new Contract(MANAGER,['function collect((uint256 tokenId,address recipient,uint128 amount0Max,uint128 amount1Max)) payable returns(uint256 amount0,uint256 amount1)'],p);
