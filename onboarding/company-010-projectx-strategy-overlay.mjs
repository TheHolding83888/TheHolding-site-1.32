...
async function main(){
  ...
  if(candidates.length<MIN_ACTIVE_NFTS)throw new Error(`Project X active-capital NFT count must be at least ${MIN_ACTIVE_NFTS}, got ${candidates.length}`);
  ...
  state.capital.positions.push({symbol:'PX-WHYPE-USDC',assetId:ASSET_ID,protocol:'Project X',chain:'HyperEVM',strategy:'WHYPE-USDC concentrated liquidity',publicLabel:'Project X · WHYPE-USDC',quantity:1,quantityMeaning:'aggregate of the current economically active Company #010 Project X WHYPE-USDC NFTs',priceUsd:round(navUsd,8),valueUsd:round(navUsd,2),priceSource:'WHYPE canonical Company #010 price + current USDC price',capitalLayer:'productiveDividend',source:'Project X enumerable NFT state + read-only full-liquidity exit simulation',principal:{WHYPE:whypePrincipal,USDC:usdcPrincipal,nftCount:perNft.length,perNft},prices:{WHYPE:round(hypePx,8),USDC:round(usdcPx,8)},ignoredNfts:ignored,admission:{activeNavFloorUsd:MIN_ACTIVE_NAV_USD,minimumActiveNftCount:MIN_ACTIVE_NFTS,actualActiveNftCount:perNft.length},capitalRule:'Count active Project X strategy NAV once. Empty/dust/other-pair NFTs are excluded from this strategy. WHYPE principal is not additive HYPE capital.'});
  recalcCapital(state);

  state.productivity.positions=(state.productivity.positions||[]).filter(x=>!['projectx_hype',ASSET_ID].includes(x.id));
  state.productivity.positions.push({id:ASSET_ID,label:'Project X · WHYPE-USDC',quantity:1,valueUsd:round(navUsd,2),referenceAprPct:null,status:'warming',source:'current economically active Project X WHYPE-USDC NFT set',referenceMetric:null,incomeMode:'separate-claimable-fees',claimableApplicable:true,methodology:'Fee tier is not yield. APR stays Pending until a reproducible first-party yield time series is bound.'});
