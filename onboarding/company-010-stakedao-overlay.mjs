...
const s=sd.result;
if(!(n(s.totalPositionUsd)>0))throw new Error('Stake DAO position USD not measured');
...
for(const sym of expected)if(!s.underlying.some(x=>x.symbol===sym&&n(x.quantity)!==null))throw new Error(`Stake DAO ${sym} missing`);
...
state.productivity.positions.push({id:'stakedao_base_curve_4pool',label:'Stake DAO · Curve 4pool Base',quantity:n(s.totalShares),valueUsd:round(s.totalPositionUsd,2),referenceAprPct:stakeApr,status:stakeApr!==null?'measured':'warming',source:s.referenceAprSource||'Curve official Base pool base APY',methodology:s.referenceAprScope||'Base pool APY only; Stake DAO boosted CRV reward APR remains separate until independently annualised.'});
...
if(stakeApr===null)state.gaps.push({id:'stakedao-base-apy',severity:'productivity',status:'warming',meaning:'Stake DAO principal and CRV claimable are measured; Curve Base pool APY was unavailable in this run.'});
