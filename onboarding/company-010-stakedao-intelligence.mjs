...
const stakePositionUsd=num(r.totalPositionUsd);
if(stakePositionUsd===null||stakePositionUsd<0)throw new Error('Stake DAO current lifecycle unresolved');
const stakeActive=stakePositionUsd>0;
const accounting=await accountantState();
const yieldData=stakeActive?await curveYield(accounting.result.gauge):null;
...
if(stakeActive){
  ...
  r.referenceAprPct=null;
  r.referenceAprStatus='not-applicable-current-principal-zero';
  r.referenceAprSource=null;
  r.referenceAprScope='No current Stake DAO principal; Reference APR is not applicable until the position becomes active.';
  r.baseApyPct=null;
  r.baseApyStatus='not-applicable-current-principal-zero';
  r.curveYield=null;
  r.source='Stake DAO RewardVault + Stake DAO verified Accountant; current principal proven zero by reconciliation';
}
recon.generatedAt=new Date().toISOString();
recon.stakeDaoIntelligence={version:'0.2-stakedao-accountant-curve-base-yield',claimableMethod:'verified-accountant-integral',yieldMethod:'curve-official-base-apy-when-principal-active',boostedRewardAprStatus:'warming'};
fs.writeFileSync(FILE,JSON.stringify(recon,null,2)+'\n');
console.log(JSON.stringify({status:'PASS',stakeDaoValueUsd:r.totalPositionUsd,stakeDaoCurrentPositionActive:stakeActive,crvClaimable:r.crvClaimable,referenceAprPct:r.referenceAprPct,referenceAprStatus:r.referenceAprStatus,referenceAprScope:r.referenceAprScope,unboostedCrvAprPct:yieldData?.unboostedCrvAprPct??null,accountant:accounting.result.accountant,gauge:accounting.result.gauge,executionAuthority:'none'},null,2));
