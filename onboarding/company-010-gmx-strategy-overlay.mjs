...
import { AbiCoder, Contract, JsonRpcProvider, formatUnits, getAddress, keccak256 } from 'ethers';
...
const POOL_AMOUNT=keccak256(coder.encode(['string'],['POOL_AMOUNT']));
...
function poolAmountKey(market,token){return keccak256(coder.encode(['bytes32','address','address'],[POOL_AMOUNT,market,token]))}
...
for(const x of gmx.results){
  ...
  if(longRaw===0n&&shortRaw===0n)throw new Error(`${x.label}: active GMX market has nonzero Company NAV but zero POOL_AMOUNT on both sides; refusing false pool-composition diagnostic`);
  if(!(grossTokenUsd>0)||!Number.isFinite(longWeight)||!Number.isFinite(shortWeight))throw new Error(`${x.label}: GMX pool composition could not be valued`);
  ...
  strategies.push({
    ...
    poolExposureDiagnostic:{
      source:'GMX DataStore POOL_AMOUNT using official Keys.poolAmountKey derivation + exact market long/short token keys; token prices are the same official Oracle inputs used for Reader valuation',
...
state.strategies.gmx={
  version:'0.1.1-company-010-gmx-strategy-intelligence-pool-key-fix',generatedAt:new Date().toISOString(),status:'measured',
...
state.provenance=state.provenance||{};state.provenance.gmxStrategy={version:state.strategies.gmx.version,generatedAt:state.strategies.gmx.generatedAt,sourceReaderVersion:gmx.version,dataStore:DATASTORE,poolAmountKeyMethod:'keccak256(abi.encode(hashString("POOL_AMOUNT"), market, token)); hashString = keccak256(abi.encode(string))'};
